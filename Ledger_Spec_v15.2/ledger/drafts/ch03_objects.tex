...
This chapter discharges constitution clauses C-4.1--C-4.11 --- the objects, the immutable log, and the signed coordinate vector C-4.10 and mass-never-value clause C-4.11 as amended in v1.2 (C-4.10 and C-4.11 jointly with Chapter~\ref{ch:collateral}).
...
unit~$u$ that wallet~$w$ holds on coordinate~$c$ under agreement~$G$. The coordinates are
the constitution's three --- \textbf{owned, lent, posted} --- and the five constitutional
names are the rays: \emph{lent out} and \emph{borrowed} are the two signs of lent,
\emph{posted as collateral} and \emph{received as collateral} the two signs of posted.
...
vector, and neither vanishes. A quantity earns a coordinate only when a distinct
...
scope passes the test, so there is no fourth coordinate --- and it earns its agreement
index for the same kind of reason: two postings under two agreements are two facts,
valued, discharged, defaulted, and returned separately, so the agreement is part of what
the coordinate \emph{is}, not a label on it.

A move writes exactly one $(c,G)$ coordinate of one unit, paired legs as always, so
conservation holds per (unit, coordinate, agreement) --- the finest grain, and the
primitive (Chapter~\ref{ch:invariants}, Theorem~\ref{thm:conservation}). The
three-coordinate vector \textbf{(owned, lent, posted)} the constitution names is the
\textbf{aggregate projection} of this finer object, summing over agreements:
\[
...
image~$0$. Constitution \S4 states the balance as the single (owned, lent, posted) vector;
that vector is this aggregate, and the (coordinate, agreement) refinement --- needed so
that two encumbrances under two agreements cannot cancel --- is stated here as primitive
and parked, with the exact amendment text, as entry PARK-3 of the parking index
(Chapter~\ref{ch:scope}). No chapter amends the constitution, which is the owner's alone.
