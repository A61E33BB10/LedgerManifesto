...
corrected prefix (Theorem~\ref{thm:timetravel}), and recomputing a view costs nothing and
waits for nothing. \emph{Money} does not. Because money already moved cannot be unsent, a
compensating transaction that repairs a balance is agreed with the counterparty and
authorised by a person before it fires. The authorisation is itself a recorded observation;
the compensating transaction carries a reference to both the correction event and its
authorisation, and the door refuses a compensating transaction that lacks the authorisation
reference. In phase~0 no correction fires without human intervention.
...
data Authorisation = Authorisation { corrects     :: EventId,  approver :: PersonId }
-- a recorded approval: agreed with the counterparty, signed by a person.
compensating :: CorrectionId -> AuthorisationId -> [Move] -> Transaction
-- a money repair references BOTH the correction and its authorisation, or the door refuses:
prop_noRepairWithoutAuth :: Transaction -> Property
prop_noRepairWithoutAuth tx = isCompensating tx ==> hasAuthorisationRef tx
\end{lstlisting}
...
coordinate, agreement), Theorem~\ref{thm:conservation} --- not at the aggregate (unit,
coordinate) grain. The distinction has teeth precisely on the cross-agreement history it
catches: a wallet posts 100 of a unit under agreement $G_1$ and receives 100 of the
same unit under a different agreement $G_2$, so the aggregate posted coordinate nets to
zero. Both the aggregate law and the finer law hold on that history --- each per-agreement
coordinate is conserved across wallets, and so is their sum --- but a report or property
that reads only the aggregate posted axis sees zero and passes \emph{vacuously}, reporting
a fully encumbered wallet as unencumbered (Chapter~\ref{ch:reporting}). The finer law keeps
$\mathrm{bal}_{\text{posted},G_1}=+100$ and $\mathrm{bal}_{\text{posted},G_2}=-100$ as two
coordinates, and the gross-encumbrance property reads them as $100$, not $0$.
...
without ever witnessing the collapse it exists to catch, and \S 2's zero-firings rule makes
that a defect, not a pass. \texttt{genCrossAgreement} therefore constructs the branch by
construction with rate $r$, set well above the floor, and \texttt{checkCoverage} turns ``the
netting branch was never generated'' into a \emph{failed} run rather than a silent green; the
witnessed firing rate is asserted at $\ge 1\%$ of generated histories. On every history that
...
--- owned $0$, received $100$, re-post attempt $200$ --- so the refusal branch is
reached with probability~$r$; setting $r$ well above the floor and wrapping the suite
in \texttt{checkCoverage} turns ``a zero-owned over-re-post was never generated'' into
a \emph{failed} run rather than a silent green, and \S 2's zero-firings rule makes that
a defect, not a pass. The witnessed refusal rate is asserted at $\ge 1\%$ of generated
histories. On every history that clears the precondition the attempted admission is
refused, because $+100 > 0$; flip the convention to received-positive and the net reads
$-100 \le 0$, the admission passes, and \texttt{prop\_coverageSignConvention} reports
the inversion the invariant exists to forbid --- while the lawful chain worked under
Invariant~\ref{inv:coverage}, received $100$ and re-post $60$, nets to $-40 \le 0$ and
is admitted.
...
One out-edge every referencing unit declares is the corporate-action edge
(Chapter~\ref{ch:objects}, Section~\ref{sec:termsheet-graphs}): across a split of a referenced
underlying, its action appends a new ProductTerms version carrying each referencing term
through the term-adjustment operator (Chapter~\ref{ch:marketdata}). The edge carries its own
executable witness --- the derivative reading of the invariance witness
(Invariant~\ref{inv:consistency}): the derivative's payoff read on the appended terms equals
its payoff read on the pre-event terms, and no guard fires that the pre-event terms would not
have fired. The precondition --- a generated derivative \emph{and} a generated corporate action
on the unit its terms reference --- is a genuine one, not a tautology.
...
When the CA-flag fires the generator constructs the split on the referenced underlying by
construction and prints the next close in the gap between the adjusted barrier $40.00$ and the
stale $80.00$, so the corrective branch --- and the phantom it would be without the edge --- is
reached with probability~$r$; setting $r$ well above the floor and wrapping the suite in
\texttt{checkCoverage} turns ``a split on a referenced underlying was never generated'' into a
\emph{failed} run rather than a silent green, and \S 2's zero-firings rule makes that a defect,
not a pass. The witnessed firing rate is asserted at $\ge 1\%$ of generated histories. On every
history that clears the precondition, the payoff on the appended terms equals the payoff on the
pre-event terms, and the stale-barrier knock --- $47.50 \le 80.00$, a payout on a stock that
never fell --- does not fire, because the live edge reads the adjusted barrier $40.00$ and
$47.50 > 40.00$. The property fails exactly on the dropped-edge design: strip the
corporate-action edge and the barrier stays $80.00$, the knock fires, and
\texttt{prop\_termAdjustmentInvariance} reports the phantom the edge exists to refuse.
...
other: its nodes are \texttt{instructed}, \texttt{settled}, and \texttt{failed}, and the
four properties above (Invariant~\ref{inv:graph-consistency}) test its three interpreters
for agreement --- no new machinery. What the generator must not leave unwitnessed is the
off-happy-path walk: the \texttt{instructed}$\to$\texttt{failed} edge, on which the same
unit \emph{becomes} the fails-cascade obligation (Chapter~\ref{ch:collateral}, \S9.3); the
\texttt{instructed}-node record-date branch on which a cum trade raises the market-claim
leg; and the \texttt{settled}-node record-date branch on which an ex trade raises the
mirror market-claim leg --- the fourth quadrant. A generator that only ever settles, or
only ever leaves an ex trade unsettled, would satisfy the four properties without
exercising these.
...
declared action --- the fails-cascade obligation, never a reversal of history. Setting $r$
above the floor and wrapping the suite in \texttt{checkCoverage} turns ``the fail branch was
never generated'' into a \emph{failed} run, and the witnessed rate is asserted at $\ge 1\%$
of generated histories, so \S 2's zero-firings rule makes an unexercised fail branch a
defect, not a pass.
...
\texttt{raisesMirrorClaim} at $\ge 1\%$ of generated histories, so \S 2's zero-firings rule
makes an unexercised fourth quadrant a blocking defect, not a silent green. The branch
...
PositionState holds not one fact per (wallet, unit) pair but a collection keyed by the
cause-derived identifier (Chapter~\ref{ch:homes}). The guarantee that makes the key
load-bearing is that two entitlements can be live on one pair at once and each retires exactly
when its own obligation discharges --- never early, never on the other's discharge. A generator
that declares only one dividend at a time would satisfy every balance property without ever
placing two facts in one slot.
...
so between $p_1$ and $p_2$ exactly one entitlement is live; setting $r$ above the floor and
wrapping the suite in \texttt{checkCoverage} turns ``two dividends in flight was never
generated'' into a \emph{failed} run rather than a silent green, and \S 2's zero-firings rule
makes an unexercised two-in-flight branch a defect. The witnessed firing rate is asserted at
$\ge 1\%$ of generated histories. On every history that clears the precondition, both
...
One cause can move many units. A single corporate action fires the contract once per
affected unit and proposes one transaction per unit --- a cascade of $n$ legs tied
together by their shared $\mathit{causeEventId}$, yet each carrying its own identifier
$\mathit{txid}=H(\mathit{causeEventId},\mathit{contractId},\mathit{unitId},\mathit{seq})$
(Chapter~\ref{ch:machines}). Idempotence keyed on the event would suppress every leg
but the first; idempotence keyed on the $\mathit{txid}$ commits all $n$ and each
exactly once. The property asserts both directions --- no leg dropped, no leg
duplicated --- by delivering the whole cascade twice, reordered.
...
it a second time in permuted order, so the twice-delivered branch is reached with
probability~$r$; setting $r$ well above the floor and wrapping the suite in
\texttt{checkCoverage} turns ``a multi-leg cascade delivered twice was never
generated'' into a \emph{failed} run rather than a silent green, and \S 2's
zero-firings rule makes that a defect, not a pass. The witnessed firing rate is
asserted at $\ge 1\%$ of generated histories. On every history that clears the
...
$(\mathit{unitId},\mathit{seq})$, and \emph{each exactly once}, because the second,
reordered delivery recomputes identifiers already on the log. The property fails on
both broken designs: key idempotence on $\mathit{causeEventId}$ alone and the second
through $n$th legs never commit (a leg dropped); drop the retry check and the second
delivery double-commits every leg (a leg duplicated).
...
The pair is tested both ways, and the second half is what keeps the first honest: without
a witnessed non-commuting case, ``swapping preserves state'' could pass on a generator that
only ever draws independent pairs, and the harmlessness precondition C-2.7 forces would be
vacuous. The first property swaps a disjoint pair and asserts the full state --- balances
and all three homes --- is identical; the second swaps a buy against a record-date event on
the \emph{same} stock and wallet and asserts the two orders reach \emph{different} state,
because the entitlement differs.
...
genDisjointPair :: Gen History  -- draw a disjoint-pair flag with rate r; when it fires,
  -- build two adjacent txs whose footprints share NOTHING, by construction:
  -- draw two DIFFERENT units, two DIFFERENT wallets, two DIFFERENT agreements, so the
  -- balance cells differ AND the UnitStatus/PositionState/ProductTerms targets differ.
...
\texttt{prop\_independentSwapInvariant} pass without witnessing a single lawful swap, and
\S 2's zero-firings rule makes that a defect. \texttt{genBuyRecordDate} constructs the
...
the two orders genuinely diverged. The rates $r$ are set well above the floor, and
\texttt{checkCoverage} asserts each firing at $\ge 1\%$ of generated histories. Together the
...
correction, and \S 2's zero-firings rule makes that a defect, not a pass. The generator is
...
When the late-flag fires the generator constructs $k<\text{end}$ by construction, so the
corrective-insertion branch is reached with probability~$r$; setting $r$ well above the floor
and wrapping the suite in \texttt{checkCoverage} turns ``the back-dated branch was never
generated'' into a \emph{failed} run rather than a silent green. The witnessed firing rate is
asserted at $\ge 1\%$ of generated histories. On every history that clears the precondition,
...
\texttt{NoDoubleCount} quantifies over units, not over one instrument, so it must be witnessed
on every kind of lifecycle event that extinguishes an exposure --- and a property whose
precondition is never generated passes without seeing anything. Two kinds extinguish. OT-1's
barrier touch is one: a triggered one-touch prices at $0$. The day's settlement of a
settled-to-market future is the other: once the variation margin is owned cash the future's
exposure is gone, so it prices at $0$ too, and marking it against the futures level would count
the day's move a second time --- exactly the double-count \S\ref{sec:pricing-state} forbids. A
collateralised-to-market future is the contrast that keeps the predicate honest: its return
obligation survives the settlement, so it is \emph{not} extinguished, and it prices against the
last settled level; the generator must produce it, and the property must not zero it. The
...
the same day applied to a future whose declared regime is collateralised-to-market. Each rate is
set well above the floor, and \texttt{checkCoverage} turns ``the STM-future branch was never
generated'' into a \emph{failed} run rather than a silent green. The witnessed firing rate of
each of the three labels is asserted at $\ge 1\%$ of generated histories, so
...
Zero firings on any of the three labels fails the suite. Because \texttt{extinguished} is an
...
The report's available-mass figure is $\max(\mathrm{bal}_{\text{owned}},0)-\sum_G
\mathrm{bal}_{\text{posted},G}$ (Chapter~\ref{ch:reporting}), the same $\max(\cdot,0)$ the
coverage invariant carries. Written without the $\max$ it goes negative for a negative-owned
wallet --- a plain short, or an issuer wallet conservation creates --- contradicting
Chapter~\ref{ch:invariants}. The property asserts non-negativity on exactly those wallets.
...
would pass \texttt{prop\_availabilityNonNegative} without ever testing the $\max$, and \S 2's
zero-firings rule makes that a defect. The property reads the Chapter~\ref{ch:reporting} availability
...
The walk \texttt{instructed}$\to$\texttt{failed} is written by the settlement-obligation unit's own
contract --- the one legal writer of its lifecycle node (Chapter~\ref{ch:homes},
Invariant~\ref{inv:one-writer}) --- on one of two recorded triggers: an external fail notice
admitted as a moveless observation, or the due-date watch finding no settlement-confirmation
observation by the deadline. The property asserts the writer and forbids an orphan failed node.
...
Sub-custody across venues is the ordinary partition of a position across wallets, not a new axis
(Chapter~\ref{ch:valuation}). The mark-to-market NAV is a fold over custody wallets, each priced at
its venue's official close; the property asserts it equals that per-wallet sum and that the total
holding is the sum of the custody-wallet balances --- no venue dimension was added to diverge from
it. It bites when two or more custody wallets hold the same unit at different closes.
...
so the cross-venue basis is non-zero and the fold is exercised; the $\ge 1\%$ floor is asserted and
\S 2's zero-firings rule makes an unexercised multi-venue branch a defect. The property fails on the
...
the effective-date reference close $C_0$ plus one close per fixing date
$C_1,\dots,C_{N}$, with fixing $i$ the return $\ln(C_i/C_{i-1})$. The property
asserts the count relation directly, that $C_0$ is on the record before fixing 1
(no return references a missing close), and that the fold sums exactly $N$ squared
returns. The precondition --- a generated variance swap --- is genuine, and every
variance-swap history witnesses it.
...
so the $\ge 1\%$ floor is met and \S 2's zero-firings rule turns an unwitnessed run
into a defect. The property fails exactly on the off-by-one design: read $N$ closes
...
Because every observation enters the log as a moveless observation-recording transaction
(Chapters~\ref{ch:objects}, \ref{ch:marketdata}, and~\ref{ch:machines}), re-folding a log prefix
reconstructs not only balances and the three homes but every recorded observation --- there is no
second store. The property discards everything but the log and rebuilds the whole record,
observations included, and compares. A generator that recorded observations outside the log --- a
second door --- would fail it at once.
...
clears by construction and the $\ge 1\%$ floor is met; \S 2's zero-firings rule turns an unwitnessed
run into a defect. The property fails exactly on the second-door design: record an observation
...
watches, state schema, and actions never disagree; the term-adjustment property holds ---
across a generated split of a referenced underlying the derivative's payoff is invariant and
no stale-barrier knock fires --- witnessed on at least $1\%$ of histories; moves are shown to
be the sole balance mutator; a duplicate firing is inert and a late firing folds in at its observation index ---
identical when reordered against unrelated events, its tail recomputed when reordered within
its unit's own sequence; firing~127 recomputes bit-for-bit; conservation holds per (unit,
coordinate, agreement) and the cross-agreement netting branch --- post under $G_1$, receive
under $G_2$, aggregate posted axis zero --- is witnessed on at least $1\%$ of histories, the
gross-encumbrance read reporting the wallet encumbered where a net read would show zero; two
dividends in flight are both live in the position-state collection and each retires on its own
payment date --- witnessed on at least $1\%$ of histories; a coordinated cascade delivered
twice commits each of its $n$ legs exactly once and all $n$ --- witnessed on at least $1\%$ of
histories; the
bitemporal pair BITEMP-1 and BITEMP-2 hold with the back-dated-observation branch
($k<\text{end}$) witnessed on at least $1\%$ of histories; and the
knock--revalue--call--discharge model holds its two safety
invariants
