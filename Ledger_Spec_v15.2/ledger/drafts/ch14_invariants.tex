...
\item \textbf{Append-only and hash-chained.} An admitted transaction is never
  ...
  names what it supersedes and carries a recorded human authorisation before it fires
...
The fork is a recorded, authorised fact. The new lineage names its parent point and carries a
human authorisation --- the forward-repair gate of Chapter~\ref{ch:testability}, raised from a
single move to a whole lineage --- and both are on the record. Which lineage is authoritative
is therefore itself a recorded fact, and exactly one is authoritative at any position.

One primitive is shared with the virtual ledger of Chapter~\ref{ch:virtual}: replay to a point
and build forward on a fresh lineage. The readings differ only in authority. A simulated
ledger branches to \emph{explore} --- its lineage is never authoritative and nothing settles
on it (Chapter~\ref{ch:virtual}, Section~\ref{sec:simulated}); a fork branches to
\emph{repair} --- its lineage becomes authoritative by the recorded authorisation. One
