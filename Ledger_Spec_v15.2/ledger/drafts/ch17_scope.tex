...
eliminations; \textbf{behavioural-event generation} --- pricing and stress paths driven only
by generated prices never exercise the events that are \emph{decisions}: a default, an
elective exercise, an election on a merger; a generator that reaches these states must model
the deciding party's behaviour, not just the market, and that model is not yet designed;
\textbf{corporate-action generation} --- a simulated corporate action is not a generated
price but a generated \emph{terms version}: the split, spin-off, or merger a path invents
must cross as a notice and drive the term-adjustment operator (Chapter~\ref{ch:marketdata})
exactly as a real one does, and the generator that produces well-typed terms versions is
open; \textbf{nested simulation} --- a simulated path that itself branches a simulated
ledger, as an XVA computation branches a pricing path off each exposure state; the seed
discipline and result provenance must compose across levels, and that composition is named
here so it cannot be improvised; and the remaining items of the companion register. Naming
...
\noindent The index is empty of open conflicts and live in its history: five conflicts were
parked with exact amendment text, five were decided by the owner, and the resolutions above
are the evidence. An empty index with no history would signal a mechanism never exercised;
this one records each exercise. A future conflict parks here the same way.
