...
immutable versioned \textit{ProductTerms}$[u]$; mutable shared \textit{UnitStatus}$[u]$; and
...
rows are retained at zero). There is no wallet-keyed economic-state sector: every per-wallet
...
Current benchmark level & \textit{UnitStatus}$[u_{\mathrm{bench}}]$ \\
\bottomrule
\end{longtable}
...
\item[\textbf{E2 --- The observation surface is unversioned and unattested (the linchpin).}]
The price vector $P_t$, the benchmark level, the TRS financing fix $r_k$, and intraday path
observations (barrier touch, realised vol) all strike real, irreversible cash, yet they enter
through \textit{UnitStatus}, which A1 defines as mutable, shared, overwritten in place, and
\emph{not} versioned. Consequences: the exact price that struck a fee is overwritten and not
replayable, so path-independence (P10) is not achievable across replay and a valued balance sheet
is only as reproducible as a mutable cell; $\mathcal{L}_v$ valuation and $\mathcal{L}_r$
settlement cannot be \emph{proved} to share one price, so divergence (hence unexplained PnL) is
representable; a base and an overlay mandate resetting at the same instant can read different
prices for the same wallet; and the benchmark and $r_k$ get \emph{weaker} discipline than $P_t$,
with no staleness, fallback, or cross-source check, though the benchmark hurdle is client money.
\emph{Direction of a fix:} prices, benchmark levels, and rates must enter as attested,
content-addressed, append-only \emph{snapshot events}, with consistency secured by sharing the
snapshot hash rather than by after-the-fact reconciliation. This single change also discharges E1,
because recorded prices are what make the economic scalars folds. It carries two governance
sub-items: the oracle-key lifecycle (generation, rotation, revocation --- on which replay
semantics depend) is unspecified; and every trust assumption at the cash boundary is currently
ownerless, which is forbidden for a value-bearing input.
...
observations and external identity. E2 is the linchpin: an attested, versioned snapshot discipline
for the observation surface discharges E1 directly and underwrites the determinism the fee, guard,
margin, and TRS settlements all silently assume. These three weaknesses are the agenda for the next
framework version.
