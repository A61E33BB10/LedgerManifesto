...
% OWNER'S RULING (2026-07-14): hard page cap raised 100 -> 110. The headroom is for
% NORMATIVE MINIMA AND THEIR WORKED EPISODES ONLY -- new invariants, guards, obligations,
% properties, and the episodes that exercise them. Elaboration, background, and operational
% detail still route to companions. CONCORDIA certifies against 110; drawing the headroom
% for anything but normative content is a veto ground.
...
\title{The Ledger\\[6pt]
\large Specification, Version 16.0}
\author{R.\ Delloye}
\date{July 2026}
...
Ledger Framework Constitution v1.3: the objects the system manipulates and the signed
...
This chapter discharges constitution clauses C-1.1--C-1.5 (the objective and the one-record diagnosis) and C-2.1--C-2.8 (the eight commitments); it states the one-way authority of C-Auth.1 and the fixed vocabulary of C-Auth.4.
...
entitled; meet the record date then buy and you are not. The log's total order is part
of the meaning of the record, never bookkeeping. This commitment forces the single
writer and the single total order of Chapter~4, and the one-event-at-a-time step of
...
the refusal to reorder anything unless harmlessness is proved (Chapter~14).
...
Framework Constitution v1.3 and against nothing else; each chapter opens by naming the
...
why: a wrong interpretation is repaired forward by an authorised compensating transaction in the open
...
This chapter discharges constitution clauses C-4.1--C-4.11 --- the objects, the immutable log, and the coordinate discipline of C-4.10--C-4.11: ownership first and alone value-bearing, further coordinates mass-only under a named agreement, and the three admission tests, against which \S\ref{sec:coordinate-vector} demonstrates the declared schema compliant (jointly with Chapter~\ref{ch:collateral}).
...
signs of posted; the rest of this section demonstrates that it passes the three tests.
...
Theorem~\ref{thm:conservation}. The earlier conflict with v1.1's fixed schema, parked as
PARK-3, is closed by the owner's ruling in v1.2; the closure and its history are recorded
in Chapter~\ref{ch:scope}.
...
perimeter being TA-ARRIVAL's matter (Chapter~\ref{ch:requirements}). The Executor stamps every arrival with its
\textbf{capture envelope} --- the delivering source and the arrival's log position, which
every push carries even when its payload is malformed --- and proposes its recording
\emph{before} payload validation. An arrival whose event kind is registered then routes as
above. One whose kind is undeclared, whose reference the record cannot resolve, or whose
payload fails validation --- provenance incomplete included --- is still recorded, under
the envelope it did carry, as a \textbf{quarantined event}: the record-then-refuse-processing
discipline of failure mode W4 (Chapter~\ref{ch:marketdata}). Never routed, never lost;
``carrying its provenance'' means at least this envelope, and the payload's own provenance
where it has one.
...
order nobody can recompute, and defaults fail closed (\S2). This refusal is
constitutional, not a design preference: C-2.7 forbids reordering anything unless
harmlessness is proved, and two non-commuting transactions with no declared
precedence are exactly the case where it cannot be. The refusal is
...
pays --- are corrected by authorised compensating transactions
(Chapter~\ref{ch:testability}, forward repair), not left as posted. Rebuilding a home is therefore a re-fold in
...
This chapter discharges constitution clauses C-8.1--C-8.9 --- NAV as a projection, PnL as NAV variation, and the three-case deposit-neutrality of \S8 as amended in v1.2 (the inflow-regime clauses C-8.6--C-8.9 jointly with Chapter~\ref{ch:collateral}).
...
Against v1.1's unconditional guarantee this off-market case was a genuine conflict,
parked as PARK-4 rather than defined away; the owner ratified the amendment on
2026-07-13 and adopted it in v1.2, and the closure is recorded in
...
This chapter discharges constitution clauses C-9.1--C-9.3 (the corporate action as a transaction, the market data operator, and its three governing principles), and C-4.12 (the event universe: finite, closed, declared event kinds with routing total at registration; jointly with Chapters~\ref{ch:machines}, \ref{ch:testability}, and~\ref{ch:requirements}).
...
(constitution C-4.12, adopted in v1.3); a kind is registered, on the record, before any
...
This chapter discharges constitution clauses C-4.10--C-4.11 (the coordinate discipline: mass-only marker planes under named agreements, jointly with Chapter~\ref{ch:objects}) and C-8.6--C-8.9 (the three inflow regimes, C-8.8's no-re-use of segregated collateral enforced by the right-of-use gate of Chapter~\ref{ch:invariants}), and C-6.5 (every right or obligation a unit) for the agreement units it introduces.
...
its mass received under $G_2$ are two distinct coordinates that never cancel. The agreement
index is therefore not new machinery --- it is the reference the type has carried on every
marker leg all along, now lifted into the coordinate rather than left beside it.
...
whose declared regime is custody --- a security interest \emph{without} right of use
(\S8 case 3, the third row above). The inflow writes marker mass on the posted plane:
...
\item net within L: $44{,}000 - 41{,}000 = \USD{3,000}$ owed to W-LEND --- a projection over
  L's netting set, the collateral already applied;
...
leg-wise and stays total over partial fills. In both cases the Transaction Executor admits the
...
This chapter discharges the auditability and reproducibility commitments C-2.1--C-2.2 for the presentation and reporting surface (jointly with Chapter~\ref{ch:objective}), building on C-1.5 (two projections cannot disagree) and C-3.5 (every view a projection).
...
This chapter discharges constitution clauses C-11.1--C-11.5 (the structural invariants) and C-12.1--C-12.5 (time travel and both honest answers, replay, idempotence, forward repair, and the authorised fork).
...
$(w,u)$ posted and lent fibres with $(w,u)$ owned, and an owned reduction symmetrically
...
one. C-12.1 (v1.3) names both honest answers and delegates their indexing to this
specification; the two axes are that indexing, and the discharge is stated at the close of
this subsection.
...
Theorem~\ref{thm:timetravel} is that discharge: the transaction-time prefix and valid-time
horizon are the indexing the constitution now leaves here, and the two both-correct answers
are its two clauses. The single-axis reading once carried as entry PARK-2 is closed by the
owner's \S 12 ruling (history in Chapter~\ref{ch:scope}).
...
asserted. C-12.1 (v1.3) names both answers and delegates their indexing here; BITEMP-1 and BITEMP-2
...
and replayability, and the following eight requirements are its minimum.
...
The boundary rests on four named trust assumptions and no other: TA-KIND, that a kind
...
The machine minima (M1--M8) are liveness and replay properties: generate restart,
duplicate-emission, and late-firing histories, and confirm that no captured event is lost, that
...
\emph{Open-Problems and Escalation Register} companion. The close-out and netting
algebra, named the top open risk since v15.0 --- per-netting-set claim units were made
first-class (Chapter~\ref{ch:collateral}) precisely so close-out is representable --- is
designed by the ruled Phase~A memo and discharged in v16.0 at Chapter~\ref{ch:collateral}
(Section~\ref{sec:closeout}): the default routes to the master agreement unit's own contract,
the netting set is the units whose declared terms name that master, the net is a projection
refused across masters at the door, the net mints as one claim (CLM-CO), each member retires
from the zero vector by close-out compensation, recovery decouples as the claim's mark walks
to a defaulted recoverability, and the par-minus-recoverable gap is recognised as lawful loss
on the supervised write-off path (C-8.4), never as error repair. It is kept named, not erased
--- an open risk that vanished without trace would be indistinguishable from one never worked. Two items named here
at the 2026-07-13 consistency review are now closed and kept named, not erased. The
\textbf{right-of-use gate on collateral re-use} --- open through v15 and \emph{live and
blocking}, because the coverage net bounded re-posting by possession alone and so admitted
a re-post of segregated mass --- is discharged in v16.0: the coverage invariant
(Chapter~\ref{ch:invariants}) is gated by the source agreement's right-of-use term,
received mass under a no-right-of-use agreement contributes zero to the net, and a re-post
of segregated collateral is refused at the door --- C-8.8 enforced by construction, not
left to an implementation. The \textbf{quantity-aware
settlement-obligation unit} --- a partial settlement leaving a settled quantity and a failed
residual the single lifecycle node could not split into legs --- is settled in v16.0 by the
partial edge that splits the unit into a settled leg and a failed-residual leg
(Chapter~\ref{ch:settlement}), each a settlement-obligation unit at its own node, so the
four-quadrant routing stays total over partial fills. Each closure keeps its history --- an
open item that vanished without trace would be indistinguishable from one never worked.
Named further: the correction
...
\item \textbf{C-4.12 (the event universe) --- CLOSED in v1.3.} No closure, no total
  routing, four loss paths on the captured side: event kinds now finite, closed, declared;
  routing total at registration. In adopting, the owner \emph{generalised} the ratified
  text --- the quarantine guarantee covers every unprocessable arrival (undeclared kind,
  unresolvable reference, failed validation), not the undeclared-kind case alone, and the
  clause states where the guarantee stops: it runs from arrival, the perimeter named as a
  trust assumption (TA-ARRIVAL, Chapter~\ref{ch:requirements}). Discharged at
  Chapters~\ref{ch:marketdata}, \ref{ch:machines}, \ref{ch:testability},
  \ref{ch:requirements} (M8).
\end{itemize}

\noindent The parking index is fully closed, and live in its history: six conflicts
parked with exact text, six decided and adopted by the owner --- five in v1.2, the sixth
in v1.3 --- the six resolutions above the evidence. A future conflict parks here the same
way.
