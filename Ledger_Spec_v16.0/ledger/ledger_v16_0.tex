...
This chapter discharges constitution clauses C-4.10--C-4.11 (the coordinate discipline: mass-only marker planes under named agreements, jointly with Chapter~\ref{ch:objects}) and C-8.6--C-8.9 (the three inflow regimes), and C-6.5 (every right or obligation a unit) for the agreement units it introduces.
...
received in enters negative --- so mass received offsets mass re-posted and re-use
of received collateral is bounded by possession; the $-40$ chain and the refused
over-re-post are worked once under that invariant. The signed net is the right
...
(Constitution~C-8.8, segregated included) must not be re-used, and the coverage net does
not yet gate re-use by the right-of-use term --- it would admit a re-post of segregated
mass. That gap is a named open problem (Chapter~\ref{ch:scope}), stated in the open;
until the gate is built, an implementation must not enable collateral re-use --- live and
blocking, not a label.
...
enables re-use until the gate is built. One item named here at the 2026-07-13 consistency
review is now closed and kept named, not erased: the \textbf{quantity-aware
...
four-quadrant routing stays total over partial fills. Named further: the correction
