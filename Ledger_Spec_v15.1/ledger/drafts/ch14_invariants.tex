...
grain forbids one class of fabrication the coarser one admits. The (coordinate, agreement)
refinement of Section~\ref{sec:coordinate-vector} is parked, with exact amendment
text, as entry PARK-3 of the parking index (Chapter~\ref{ch:scope}); no chapter amends the
constitution.
...
symmetry of the log preserves. Reorder any two independent transactions --- ones
that share no (wallet, unit, coordinate, agreement) --- and every coordinate sum is
unchanged, because each is a sum of paired-leg contributions that commute.
Permutation invariance of independent moves and the vanishing coordinate sum are
one fact seen twice; the first is itself a checkable (metamorphic) property.
