...
This chapter discharges constitution clauses C-8.1--C-8.9 --- NAV as a projection, PnL as NAV variation, and the three-case deposit-neutrality of \S8 as amended in v1.1 (the inflow-regime clauses C-8.6--C-8.9 jointly with Chapter~\ref{ch:collateral}).
...
Constitution~\S8 asserts deposit-neutrality \emph{unconditionally}: a financing
received against an equal obligation created in the same transaction ``cannot
change net owned value.'' That claim is a theorem in one case and false in
another. The gap is a genuine conflict with the constitution, parked openly below
...
When the declared rate is off-market, it is not. Receive \USD{100} of cash
against an obligation whose fair value is \USD{98} --- financing taken below the
fair rate --- and net owned value rises by \USD{2} at receipt: NAV moves. That
\USD{2} is a day-one \emph{financing basis}, the correct measurement of the
off-market terms; but naming it does not make the inflow neutral. The
constitutional guarantee, asserted for every case, does not hold here: it is
narrowed to fair-value financing.

This is a genuine conflict between the specification and Constitution~\S8. It was
parked --- not resolved, and not relabelled --- as entry PARK-4 of the parking
index (Chapter~\ref{ch:scope}), and the owner has since \textbf{ratified} the
amendment (2026-07-13): \S8 case~2 becomes conditional on the obligation being
valued at fair value, with any day-one difference recognised as financing basis, so
this proposition stands as stated. The ratified clause C-8.7 awaits the owner's
adoption into the authoritative manifesto, the owner's sole act; until that adoption
the manifesto reads v1.1 and this proposition records the resolved delta in the open,
never defined away.
\end{proposition}
