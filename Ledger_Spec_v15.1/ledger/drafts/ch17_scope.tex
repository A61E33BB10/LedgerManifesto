...
improvised. Named alongside it: the correction
...
\emph{Current status: the index carries four parked entries.} Adversarial review found
four points where this specification must depart from Constitution v1.1, and each is parked
here with the exact clause it touches; the exact proposed amendment text is carried in the
companion register. None is fudged, and none amends the constitution: each hands the owner a
...
\item \textbf{PARK-4 (Constitution \S8).} Section~8 asserts deposit-neutrality
  unconditionally, yet financing received at an off-market rate moves net owned value at
  receipt by a day-one financing basis. The parked amendment makes the guarantee conditional
  on fair-value measurement. Stated openly at Chapter~\ref{ch:valuation}.
\end{itemize}

\noindent The index is non-empty and live: an empty index across a specification this size
would signal that the mechanism had never been exercised, not that no conflict exists.
