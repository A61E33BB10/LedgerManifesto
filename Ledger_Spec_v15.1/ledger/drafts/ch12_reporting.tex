...
question, answered by the coverage net $\mathrm{bal}_{\text{owned}}-\sum_G
\mathrm{bal}_{\text{posted},G}$ (Chapter~\ref{ch:invariants}), and it is
non-negative because the coverage invariant bounds posting by ownership; the report
cannot show negative free assets because the door refused the state that would
produce one. Encumbrance sums gross, availability nets --- the two are never one
figure.
