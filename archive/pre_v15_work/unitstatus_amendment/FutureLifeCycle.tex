...
\emph{UnitStatus}    & $u$            & projection of the log, overwritten in place as a read cache; shared, read identically by every holder; registration-total --- every change caused by a logged event, rebuilt exactly by replay \\
...
Settlement at price $S$, multiplier $\mult$, over the current holders. The settlement price $S$ is an
externally-sourced value; it enters only as the \code{SettleVM} observation event in the log ---
captured there as a logged observation --- so a fresh replay rebuilds the exact mark and the fold stays
pure at the boundary. For each holder the reset
...
\item \textbf{Deterministic replay as a fold (C1(b)).} \code{replay} folds the
event stream over the ledger; $\code{replay}(xs\mathbin{+\!\!+}ys)=\code{replay}\,xs \mathbin{>\!=\!>}
\code{replay}\,ys$ --- replaying two batches of events in sequence gives the same result as replaying
them together, where $f \mathbin{>\!=\!>} g$ runs the error-returning step
$f$, feeds its result to $g$, and stops at the first error. Checkpoint independence is a consequence of
this law, not a test; the monotone
