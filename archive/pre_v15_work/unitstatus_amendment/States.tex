...
Status is one stage of the unit's lifecycle. UnitStatus holds one value per unit,
shared --- read identically by every holder. Its value changes over time, but
UnitStatus is not a separate source of truth: the immutable event log is. Every change
is caused by a logged event; the stored value is overwritten in place only as a read
cache. Replaying the events up to any point rebuilds the exact value that held then, so
nothing is lost by overwriting and there is no other way to change the value. A value
exists from registration onward. The price a settlement fixes enters only as the
\texttt{Settled} event in the log --- captured there as a logged observation --- and is
data the stage carries: it rides
...
status, and positions, so every view is a projection of the stream. In particular the
stored \texttt{UnitStatus} is a cache the log can always rebuild: replaying the events
up to any time reconstructs the exact status that held then --- total on well-formed
prefixes, deterministic, and reconstructible --- so overwriting the cell loses nothing
the log does not retain, and a logged event is the only thing that can change it. Row
