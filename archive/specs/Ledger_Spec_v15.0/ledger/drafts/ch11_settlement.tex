...
variance points at \USD{1{,}000} per point. Through its life each fixing is an
observation the ledger records, not a transfer (Chapters~\ref{ch:marketdata}
and~\ref{ch:valuation}); none crosses the boundary, and each projects to the empty
list. At expiry the realised variance is $441$ points and the contract proposes one
...
the paying agent pays against. Two cases follow, and only one raises a claim.
...
holds it negative, the cum buyer's claim-for-equivalent on one side and the
deliverer's duty to remit on the other, created over the gap by the record-date firing
and discharged when the deliverer receives the distribution.
...
ex-date, and the recorded settlement state, and proposes the reconciling leg --- where
the cum case calls for one --- as one component of the determination transaction, which
the Transaction Executor admits like any other. The projection then reads the resulting
committed moves; it originates none of them.
...
trade's cum/ex status against the recorded ex-date, and the recorded settlement state
at the record date. Determination reads owned; a market claim arises in exactly one
case --- a \emph{cum} trade, executed before the ex-date, still unsettled across the
record date --- where the paying agent credits the deliverer and a market-claim unit
carries the reconciling leg from the deliverer to the cum-buyer. An \emph{ex} trade,
executed on or after the ex-date, unsettled at the record date raises no buyer claim:
the registered holder keeps the distribution. No distribution reaches any wallet except
by a recorded move against a determined entitlement or the discharge of a market-claim
unit.
\end{proposition}
...
otherwise identical trade executed on or after the ex-date would be \emph{ex} and raise
no market claim. (The cash consideration settles symmetrically and does not bear on the
entitlement.)
...
the trade's cum status against the ex-date and the recorded settlement state, proposes
the reconciling leg. One transaction:
\begin{enumerate}[label=(\arabic*),nosep]
...
\item the sale is cum and unsettled across the record date, so a market claim arises:
the paying agent will credit the record holder, the deliverer W-ALPHA. Issue the
market-claim unit MC-1,
...
across the record date, the same market-claim unit carries the mirror obligation, its
signs reversed --- one mechanism, total over the gap.
