Verified anchors against the bundle (`reference/Ledger.hs:290–311, 329–358`; `drafts/sec05.tex:29–57`; `drafts/sec06.tex:99` region; `drafts/appD.tex:44–57`; `drafts/sec07.tex:93–200`). The ruling and converged design follow.
...
**Disposition:** CONVERGED. One design, zero undischarged MATERIAL objections. Part III is the discharge register, itemised.
...
**I.2 Duplicates struck.** Thirty-six material objections reduce to twenty defect classes (Part III); e.g. the P6-idempotence break is one defect filed twice (vs THORP and NOETHER), the unit-blind index three times (vs NAZAROV, NOETHER, MILEWSKI).
...
**Definition 2 (basis point, chain, epoch).** A *basis point* of `u` is either the registration point `⊥_u` or a boundary id of `u`. The *basis chain* of `u`, relative to a replay view, is the sequence of `u`'s boundary events in **effective order** known to that view. The *epoch ordinal* of a basis point is its position in the chain — a **derived, per-view quantity**, never stored. **Stamps and state store basis-point ids, never ordinals.** This is the arbiter's one construction not present in any single memo, forced by the objections: ordinals renumber under retro-effective insertion (objections T2, N7); content-addressed ids do not. *(Source: THORP's foreign-key idea + NAZAROV's content-addressed envelope, hardened.)*

**Definition 3 (the `UnitStatus` field).** `UnitStatus` gains one field, `usBasis :: BasisPoint`, written only through the closed `StatusWrite` set (`reference/Ledger.hs:299–311`) by a new constructor:

```haskell
SetBasis BasisPoint    -- ABSOLUTE, not an increment: last-write-wins, idempotent (P6)
```

Re-applying `SetBasis b` to a status already at `b` is the identity, exactly as `SetLastSettle`/`SetSupersededBy` behave (`Ledger.hs:301`: "Last-write-wins, idempotent (P6)"). *(Source: MILEWSKI §4.1, verbatim in discipline; discharges T1/N1.)* The chain is a projection of the log; the counter is not stored at all, so chain and coordinate cannot desynchronise — one source of truth, one stated invariant: **`usBasis` equals the last effective element of the chain**, maintained by construction because both are folds of the same events through `applyTx`. *(Discharges M6.)*

**Atomicity and the weld.** A basis-changing CA is **one atomic Transaction** (C3) carrying its entitlement moves, its `SetBasis`, and its declaration. Additionally — closing NAZAROV's gap Z2 — `applyTx` **admits** the transaction only if the moves and the declaration jointly satisfy the *invariance identity* (§2, Theorem 1): quantity legs equal to `q ↦ q·f` up to explicit cash-in-lieu legs, and booked cash legs equal to `q·c`. This is checkable from the transaction alone, requires no market data, and is the same admission idiom as `move`'s positivity gate (`Ledger.hs`, Part B). Consequence: "positions doubled but basis not advanced", "basis advanced but declaration missing", and "declaration inconsistent with moves" are all **unrepresentable ledger states**. *(Sources: all four for atomicity; THORP §2 for the identity; NAZAROV Z2 forced the weld.)*
...
*(Sources: the GADT shape from NOETHER §3 and THORP §4 Tier 1; the unexported-constructor repair from NOETHER/MILEWSKI — discharging T9.)*

**Tier 2 — the normative seam (runtime epochs, skolem sameness).** The one constructor of coherent read scopes, mirroring `applyTx` as the one write door. Three repairs relative to the filed designs are load-bearing: the ledger is **sealed inside** the scope (no `Ledger` parameter downstream — discharging M1); the skolem `b` denotes a whole **basis assignment** `β : Scope → BasisPoint`, not one unit's epoch, and every accessor is unit-keyed (discharging Z3, N2); the per-unit price accessor is **fallible**, so quarantine is representable (discharging T8):

```haskell
data Snapshot b   -- ABSTRACT. Constructed only by withSnapshot, which fixes the
                  -- target assignment β = β_t|scope from replayed UnitStatus,
                  -- reads balances from the SAME sealed ledger, and transports
                  -- every admissible stamped observation to β along declared
...
2. *The CA.* One atomic transaction `τ`: moves +1,000 from the CA virtual wallet (1,000 → 2,000); `SetBasis b₄`; declaration `{KSpot ↦ Scale ½, …}`. **Admission weld:** quantity legs `2,000 = 1,000 × 2` ✓, cash legs `0 = 1,000 × 0` ✓ — Theorem 1's hypotheses checked at the door. No reachable state has the doubling without the boundary.
...
7. *Late notice.* If `τ` is unlogged at `t_price`, C3 keeps moves and boundary together, so the snapshot is coherent at `b₃`: `1,000 × 100 = €100,000` — right under the information held. Meanwhile the exchange disseminates ~50: W3 fires (innovation with no committed boundary), quarantines the series; W1 blocks settlement-grade use. At `t_notify`, `τ` commits with economic time `t_eff`; re-stamp events move the window's observations to `b₄`; corrected replay is coherent end to end. At no point does `2,000 × 100` or `1,000 × 50` reach a committed `Cash`.
...
- **sec04** — `usBasis :: BasisPoint` in `UnitStatus`; `SetBasis` (absolute) in `StatusWrite` with its C11 writer rows (CA lifecycle handler; basis-notice handler; the W2′ election event's writer row — closing THORP's stylistic gap); `usLastSettle → Maybe (Price, BasisPoint)`; one paragraph naming the observation projection; new condition **C13 (basis edge)**: every consumed datum names its joint basis; consumption requires pointwise agreement with the prevailing assignment or a ledger-authored arrow — the data-plane sibling of P3.
...
- **sec07** — snapshot versioning acquires the second axis (stored in observation basis, consumed at the prevailing assignment); time-travel items 2 and 4 extend to data, discharged by §6; the oracle's determinism contract includes the basis coordinate.
...
- **appB/appE** — P8 extended (`clone_at(t)` rebuilds `usBasis` and the observation projection); new oracles: **P24 basis coherence** (accepted valuations consumed pointwise-equal bases — structurally guaranteed; the oracle witnesses the erased boundary); **value invariance** (the weld: `value_after = value_before − booked_cash` across every boundary — the oracle that makes (a)–(c) falsifiable); **composition order** (the 49-vs-50 discriminator on ≥ 2-CA windows); **fail-closed** (undeclared kind cannot cross — a `Left`, never a pass-through); **re-stamp round-trip** (as-known vs corrected replays differ exactly on the window); **stamp round-trip** (invertible arrows compose to identity).
- **appF** — Observable row: "Observable → stamped observation (basis-disciplined)".
- **sec01/sec19** — Property 5 restated as Theorem 1 with the weld as proof; sec01's second axis named; the sec19 flagged-items register gains the closure entry, **TA-BASIS** in the trust registry (owner, consequence, detection, `N_CA` assigned by data governance), and one honest residual: the interaction of retro-effective basis corrections with the standing bitemporal open item.
- **sec13** — `forget` extended per §9.
- **reference/Ledger.hs** — §4's modules beside `FieldWrite`; the Tier-1 GADT with unexported constructors as the didactic companion.

**Invariants restated:** none weakened. P5 becomes a theorem on both factors; P6 unchanged (all writes absolute); P8 extended; P10 gains its honest hypothesis with proof unchanged; C13 and P24 are additions. Every strengthened statement was previously true only under an assumption no text stated.
...
| 10 | Z4, T9, M4 | Forgeable arrows / stamps / Tier-1 constructors | All constructors unexported; sole authors: the ledger fold, `ingest`, `withSnapshot`, `carry` (§3, §4) |
...
| 17 | N9 (+ Z-stylistic) | "Does not compile" overstates a static claim | Honest two-tier statement: sameness-not-value skolem normatively; Tier-1 GADT labelled didactic; runtime witness at the erased boundary, C11 register (§4) |
...
| 19 | M5, M6 | `arrow` non-total on adversarial `Int` epochs; chain-contiguity invariant unstated | No arithmetic indexing anywhere: resolution is log lookup keyed on boundary ids; the counter is not stored — `usBasis` = last chain element is a stated, by-construction invariant (§1, §2) |
...
**Protected elements:** the atomic move primitive, the conservation law, and the immutability of the event log are unchanged in obligation; every addition is an appended event class, a new `StatusWrite` constructor, a named projection, or a type refinement at the seam.

*"The pairing is the invariant; type both of its factors in the same frame, and the phantom PnL has nowhere to live."* — adopted from NOETHER as the design's epigraph, now with the constructors to prove it.

— FORMALIS, converged. Zero undischarged material objections.