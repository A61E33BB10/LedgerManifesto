...
\begin{remark}[Conservation as an algebraic identity]
\emph{Optional reading. The rest of the specification does not depend on it.}

\noindent\emph{Plain reading:} A move debits one wallet and credits another by the same amount, so for every unit the wallet balances sum to zero---for any list of moves, the empty list included. The books close because each move carries its own equal-and-opposite pair, not because a total is checked afterwards.

\noindent\emph{Optional categorical lens:} The column-sum \texttt{colSum\_u : Balances -> Qty} is a group homomorphism into the additive abelian group \texttt{Qty}; precomposed with the monoid homomorphism \texttt{balances = foldMap contribution} from the free monoid of moves, the composite is a monoid homomorphism that is identically \texttt{mempty}---conservation holds because each move contributes the inverse pair \texttt{(qneg q) <> q = mempty} by the group law, with no runtime sum compared to zero and no validation gate.
\end{remark}

\input{drafts/fig_conservation}
