...
event log is (justified by the idempotent replay of Section~\ref{sec:lifecycle}, \S7).
...
\emph{Plain reading.} Every view --- balances, valuation, and \emph{UnitStatus} itself ---
is computed by folding over the one event stream; none is a second source of truth, and
replaying the events rebuilds each exactly.

\emph{Optional lens.} \texttt{balances = foldMap contribution} is a monoid homomorphism ---
