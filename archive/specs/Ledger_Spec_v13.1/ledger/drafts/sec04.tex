...
Fungibility fixes a different identity key for each instrument class.
...
Instrument master data: ISIN, exchange, contract specification, issuer, coupon schedule. It is read from exchanges, vendors, and CSD records. The Ledger consumes reference data; it does not author it. CDM: \texttt{TransferableProduct} for securities, \texttt{NonTransferableProduct} for derivatives and structured products.

\paragraph{Tier 2 --- Product Registry. \emph{What lifecycle logic governs this kind of instrument?}}
One smart-contract template per product \emph{type}, not per contract. ``European equity index option'' maps to one \texttt{OptionPayout} template, and that template covers every such option regardless of underlier, strike, or expiry. CDM \texttt{ProductQualification} classifies \texttt{EconomicTerms} into a product type; each type maps to a template (Section~\ref{sec:contracts}). An entry is created automatically the first time a Tier-3 registration encounters a new product type. Thereafter the entry is immutable.
...
\emph{UnitStatus} is a projection, not a mutable store. It is one shared value per unit: a read cache of the immutable event log, rebuilt exactly by replay (established in \S\ref{sec:states-unitstatus}).
...
Registration is atomic: a unit is fully validated and added, or rejected with no partial state. Units are never deleted. A unit at end of life reaches a terminal stage in \emph{UnitStatus} (matured, terminated, settled), and its entry remains for audit and time travel. The executor rejects new moves against a non-\texttt{ACTIVE} unit, except to settle existing obligations.

Two disciplines bind registration: it writes the first value of every per-unit map, and it happens once.
...
The registering function writes the \emph{singleton} first version of \emph{ProductTerms} and the default \emph{UnitStatus} together, in one operation. It returns the updated ledger, or rejects an already-registered id with a \texttt{Left} (C10).

\emph{ProductTerms} is a \texttt{NonEmpty} list of versions, so a registered-but-versionless unit is unrepresentable, and the in-force-version accessor is total: it needs no \texttt{Maybe} for emptiness (C7).

\texttt{register} is exported; \emph{ProductTerms} and \texttt{Ledger} are exported abstract, with no field setter. That single write is therefore the only origin of a unit's terms, and no caller can falsify the \emph{ProductTerms}--\emph{UnitStatus} coupling. In code, registration is a move-less transaction applied through the same function as trades, settlements, and amendments:
...
A first registration succeeds. The same id a second time is rejected, and the prior ledger and its live state are left untouched. The rejection is a hard error, never a silent reset:
...
Conservation does not arise at registration. A registration is a move-less transaction, so its edge-sum is \texttt{mempty}, which is vacuously conserved, and the transaction touches no \emph{PositionState} row. A non-vacuous balance, and the conservation that constrains it, appears only when the first position is taken (Section~\ref{sec:states}).
...
\item \textbf{Term consistency}: static terms cohere. Expiry is in the future at creation, the multiplier is positive, the currency code is valid.
\end{enumerate}
...
Invalid units cannot enter $\mathcal{U}$, and moves cannot reference non-existent or retired units. With the conservation law (Section~\ref{sec:ledger}) and the executor's purity (Section~\ref{sec:lifecycle}), two-stage validation completes the correctness chain. The ledger operates only on validated, contract-governed units.
...
These guarantees make referential integrity (P3, Section~\ref{sec:invariants}) hold by construction: $m.\text{unit} \in \mathcal{U}$ because the executor consults the store before accepting any move.
...
CDM has no native catalogue concept. It models individual trades, not registries. It serves both listed and OTC derivatives through one \texttt{NonTransferableProduct} type, with no dedicated listed-contract-specification type and no model for exchange reference-data feeds (listings, expiry calendars, lot sizes). These are documentation gaps, not design incompatibilities. The store uses CDM objects as its internal representation; catalogue management sits alongside CDM as a Ledger-specific concern.
...
wallets. From that primitive the following hold by construction. Each is catalogued, with
its executable oracle, in \S\ref{sec:invariants}.
