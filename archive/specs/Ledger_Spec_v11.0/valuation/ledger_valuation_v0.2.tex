...
Version~0.2 adds a new section on \emph{consistency with the lifecycle engine}.  The valuation stack and the lifecycle engine both depend on and modify the same state: the price function depends on unit state ($P_t(u) = P(u, \mathrm{state}_t(u), \mathrm{market\_data}_t)$), lifecycle events create price discontinuities that invalidate the Taylor approximation, and the ordering between lifecycle processing and valuation cycles is load-bearing.  The new section formalises the lifecycle-valuation ordering protocol, defines when lifecycle events trigger immediate repricing versus staleness marking, extends the Pricing DAG with state-dependency edges, and provides two worked examples---a bond coupon payment and an SBL fee accrual---that trace the complete sequence from lifecycle event through Taylor invalidation, reprice, PnL explain, and publication.
...
Lifecycle engine~\cite[\S7]{ledger} & Read (state) + receive (signals) & Valuation reads unit state for pricing inputs (Section~\ref{sec:lifecycle-consistency}).  Lifecycle workflows signal pricing workflows after material events, triggering immediate repricing and Taylor invalidation. \\
\bottomrule
\end{tabular}
\end{center}

\noindent The valuation stack does NOT write to the move stream.  It produces ValuationRecords that are consumed by the ledger's PnL computation.  The separation is strict: moves record economic activity; valuations record price observations.  The lifecycle engine is a \emph{state provider and event source} for the valuation stack: it determines the unit state that the pricer reads (Section~\ref{sec:lifecycle-consistency}), and its events trigger repricing.
...
The valuation stack and the lifecycle engine~\cite[\S7]{ledger} operate on the same state: wallet balances, unit state dictionaries, and price vectors.  The lifecycle engine \emph{mutates} unit state (coupon flags, exercise status, accumulated cost) and generates moves (cash flows, position changes).  The valuation stack \emph{reads} unit state and market data to produce prices.  Version~0.1 of this document treats the price function as $P_t(u)$---a function of the unit and time.  This is incomplete.  The price function depends on the unit's lifecycle state, and lifecycle events create discontinuities that the Taylor approximation cannot predict.

This section formalises the consistency requirements between the two systems.

%----------------------------------------------------------------------
\subsection{The Price Function Depends on Unit State}
\label{sec:price-state-dep}
%----------------------------------------------------------------------

The Ledger specification defines the lifecycle function as a pure function~\cite[\S7.4]{ledger}:
\[
f : (\text{unit}, \;\text{state}_t(u), \;\text{market\_data}) \to (\text{moves}, \;\text{new\_state})
\]
The purity principle requires that all inputs are explicit.  The same principle applies to the price function:

\begin{principle}[State-Dependent Pricing]
\label{princ:state-dep-pricing}
The price function is:
\[
P_t(u) = P\bigl(u,\; \mathrm{state}_t(u),\; \mathrm{market\_data}_t\bigr)
\]
where $\mathrm{state}_t(u)$ is the unit state dictionary from the lifecycle engine at time~$t$.  The pricer receives both market data AND unit state as explicit inputs.  A pricer invocation without the current unit state is ill-defined.
\end{principle}

\noindent The scalar compatibility principle (Principle~\ref{princ:scalar-compat}) is preserved: the \texttt{dirty\_price} field of the ValuationRecord is still the $P_t(u)$ consumed by the PnL computation.  What changes is the \emph{input specification} of the pricer: it must read the unit state before computing the price.

\paragraph{Examples of state-dependent pricing.}

\begin{center}
\small
\begin{tabular}{l p{4cm} p{5.5cm}}
\toprule
\textbf{Instrument} & \textbf{State field read by pricer} & \textbf{Effect on price} \\
\midrule
Bond & \texttt{coupon\_paid} flags, \texttt{matured} flag & Accrued interest resets to zero after coupon payment; dirty price drops by coupon amount \\
Callable bond & \texttt{call\_exercised} flag & If called, price collapses to redemption value; if not called, price includes call optionality \\
Barrier option & \texttt{barrier\_breached} flag & Knock-out: price drops to zero if breached; knock-in: option activates if breached \\
Futures & \texttt{accumulated\_cost} (per-wallet) & Economic value = accumulated\_cost + qty $\times$ P $\times$ mult \\
SBL loan & Outstanding quantity, fee rate, accrued fees & Loan value depends on current loan size (changes on partial return) and accrued fee payable \\
Option & \texttt{lifecycle\_stage} (ACTIVE vs.\ EXERCISED) & Exercised: price = intrinsic value; expired: price = 0 \\
\bottomrule
\end{tabular}
\end{center}

\noindent The pricer's \texttt{pricing\_dependencies()} method (Section~\ref{sec:dag-construction}) already declares market data dependencies.  State dependencies do not need to be declared in the same way: the unit state is always local to the unit being priced and is read from the ledger view via the pure accessor \texttt{view.get\_unit\_state(unit)}~\cite[\S7.3]{ledger}.  State dependencies do not create inter-unit edges in the Pricing DAG (Section~\ref{sec:state-dag-edges}).

%----------------------------------------------------------------------
\subsection{The Lifecycle-Valuation Ordering Protocol}
\label{sec:ordering-protocol}
%----------------------------------------------------------------------

Consider an end-of-day sequence for a bond:
\begin{enumerate}[nosep]
\item Coupon payment lifecycle event fires: the executor processes the cash move, the unit state marks the coupon as paid, accrued interest resets.
\item Valuation cycle fires: the pricer computes the new dirty price.
\end{enumerate}

\noindent If step~(2) executes before step~(1), the pricer computes a dirty price that includes the coupon---but the coupon will be paid moments later.  The resulting PnL explain shows unexplained PnL equal to the coupon amount: the price dropped by the coupon, but the explain has no record of the coupon event.

\begin{principle}[Lifecycle-Before-Valuation]
\label{princ:lifecycle-before-val}
For any unit $u$, all pending lifecycle events for $u$ must be processed by the executor before the next valuation cycle for $u$ begins.  The pricing workflow for $u$ must not fire until it has received both:
\begin{enumerate}[nosep]
...
\paragraph{Implementation via Temporal signals.}  The lifecycle workflow for a unit signals the pricing workflow after each lifecycle event:

\begin{lstlisting}
-- In the lifecycle workflow, after executor commit:
await ExecutorCommit(pending_tx)
workflow.SignalExternalWorkflow(
    pricing_workflow_id(unit_id),
    "lifecycle_event_completed",
    {event_type: "COUPON_PAYMENT",
     unit_id: unit_id,
     timestamp: event_timestamp,
     invalidates_greeks: True})
\end{lstlisting}

\noindent The pricing workflow extends its wait condition:

\begin{lstlisting}
PricingWorkflow(unit_id, cadence, dependencies):
    ...
    lifecycle_ch = workflow.GetSignalChannel(
        "lifecycle_event_completed")
    pending_lifecycle = False

    -- In the selector loop:
    selector.AddReceive(lifecycle_ch, func(signal):
        pending_lifecycle = True
        if signal.invalidates_greeks:
            invalidate_greeks_cache(unit_id)
    )

    -- Before pricing: drain lifecycle signals
    while lifecycle_ch.HasPending():
        signal = lifecycle_ch.Receive()
        pending_lifecycle = True
        if signal.invalidates_greeks:
            invalidate_greeks_cache(unit_id)

    -- Proceed with pricing using post-event unit state
    mkt_snap = await RetrieveMarketData(unit_id, dependencies)
    ...
\end{lstlisting}

\noindent The key property: after draining all pending lifecycle signals, the pricing workflow reads unit state that reflects all committed lifecycle events.  The pricer sees the post-coupon state, not the pre-coupon state.

\paragraph{Consistency guarantee.}  The ordering protocol ensures that the valuation cycle observes a consistent snapshot of unit state.  This is not eventual consistency---it is causal consistency: the pricing workflow's read of unit state is causally ordered after the lifecycle workflow's write.  Temporal's signal delivery guarantees at-least-once delivery; the lifecycle event's idempotency~\cite[\S7.1.2]{ledger} ensures that duplicate signals are harmless.

%----------------------------------------------------------------------
\subsection{Taylor Approximation Invalidation on Lifecycle Events}
\label{sec:taylor-invalidation}
%----------------------------------------------------------------------

The Taylor approximation (Section~\ref{sec:approx-pricing}) works for smooth price changes driven by market moves.  Lifecycle events create \emph{discontinuities}:

\begin{center}
\small
\begin{tabular}{l l l}
\toprule
\textbf{Lifecycle event} & \textbf{Price discontinuity} & \textbf{Nature} \\
\midrule
Coupon payment & Dirty price drops by coupon amount & Step function \\
Ex-dividend & Equity price drops by dividend amount & Step function \\
Option exercise & Price $\to$ intrinsic (or zero if OTM) & Regime change \\
Option expiry & Price $\to$ 0 & Terminal \\
Futures daily settle & Accumulated cost resets; VM cash move & Reset \\
SBL partial return & Loan size decreases; value changes & Quantity change \\
Callable bond call & Price $\to$ call price & Regime change \\
Barrier knock-in/out & Price jumps (activation or deactivation) & Regime change \\
\bottomrule
\end{tabular}
\end{center}

\noindent These discontinuities are not captured by Greeks.  The Greeks $\delta$, $\Gamma$, $\nu$, $\Theta$, $\rho$ are partial derivatives of the price with respect to \emph{market data} observables.  A coupon payment is not a market data move---it is a state transition.  After a lifecycle event, the Taylor approximation from the pre-event Greeks is invalid: the base price $P_{\mathrm{full}}$ has jumped, and the Greeks (computed at the pre-event state) no longer describe the price surface around the post-event state.

\begin{principle}[Lifecycle Invalidation]
\label{princ:lifecycle-invalidation}
When a lifecycle event fires for unit $u$, the approximation service must:
\begin{enumerate}[nosep]
\item Detect that a lifecycle event has occurred (via the lifecycle signal on the pricing workflow, or via the event journal).
\item Invalidate the cached Greeks for $u$.  The cached FIRM ValuationRecord is no longer a valid base for Taylor expansion.
\item Return \texttt{quality = STALE} for $u$ until the next full reprice completes.
\item Optionally, for known discontinuities with closed-form adjustments, apply an immediate correction:
\[
P_{\mathrm{adjusted}}(u) = P_{\mathrm{last\_firm}}(u) - \mathrm{discontinuity\_amount}
\]
with \texttt{quality = APPROXIMATE} and a flag indicating the adjustment source.
\end{enumerate}
\end{principle}

\paragraph{Closed-form adjustments for known discontinuities.}  For deterministic lifecycle events~\cite[\S1.2, Property~5]{ledger}, the price discontinuity is known exactly:

\begin{center}
\small
\begin{tabular}{l l}
\toprule
\textbf{Event} & \textbf{Closed-form adjustment} \\
\midrule
Coupon payment & $P_{\mathrm{adj}} = P_{\mathrm{last}} - \mathrm{coupon\_amount}$ \\
Dividend (ex-date) & $P_{\mathrm{adj}} = P_{\mathrm{last}} - \mathrm{dividend\_amount}$ \\
Futures settlement & $P_{\mathrm{adj}} = P_{\mathrm{last}}$ (no price change; VM is a cash flow) \\
\bottomrule
\end{tabular}
\end{center}

\noindent For events involving optionality (exercise, barrier breach, callable bond redemption), no closed-form adjustment exists: the price after the event depends on the exercise outcome or barrier status, which requires a full reprice with the post-event unit state.  For these events, the approximation service returns STALE until the full reprice completes.

%----------------------------------------------------------------------
\subsection{State-Dependency Edges in the Pricing DAG}
\label{sec:state-dag-edges}
%----------------------------------------------------------------------

The Pricing DAG (Section~\ref{sec:pricing-dag}) currently represents two types of edges: market data dependencies (unit $\to$ observable) and unit-to-unit dependencies (structured note $\to$ embedded option).  Lifecycle state introduces a third category of dependency: the pricer for a callable bond reads \texttt{call\_exercised}; the pricer for a barrier option reads \texttt{barrier\_breached}; the pricer for an SBL loan reads the outstanding quantity.

These state dependencies differ from market data dependencies in three ways:

\begin{enumerate}
\item \textbf{Locality.}  State dependencies are local to the unit: a bond's pricer reads the bond's own unit state, not another unit's state.  They do not create inter-unit edges in the DAG.

\item \textbf{Change frequency.}  Market data changes continuously (tick-by-tick); unit state changes discretely (at lifecycle events).  The state is constant between lifecycle events.

\item \textbf{Trigger mechanism.}  Market data changes are detected by the pricing workflow via upstream signals from market data nodes.  State changes are detected via lifecycle signals (Section~\ref{sec:ordering-protocol}).
\end{enumerate}

\begin{definition}[Extended Pricing DAG]
\label{def:extended-dag}
The extended Pricing DAG $G' = (N, E_M \cup E_S)$ distinguishes two edge types:
\begin{itemize}[nosep]
\item $E_M$: market data dependency edges (as in Definition~\ref{def:pricing-dag}).
\item $E_S$: state dependency edges, connecting a unit node to its own unit state.  These are self-loops in the DAG (from the unit's state to the unit's pricer).
\end{itemize}
State dependency edges do not affect the topological order (they are self-referential, not inter-unit).  They serve as documentation of what the pricer reads and as input to the lifecycle-triggered repricing policy (Section~\ref{sec:lifecycle-repricing}).
\end{definition}

\paragraph{Practical implication.}  The \texttt{pricing\_dependencies()} method on each smart contract is extended to declare state dependencies alongside market data dependencies:

\begin{lstlisting}
CallableBond.pricing_dependencies():
    return PricingDeps(
        market_data = {issuer_yield_curve, USD_rate_curve},
        state_fields = {call_exercised, coupon_paid_flags,
                        matured})

BarrierOption.pricing_dependencies():
    return PricingDeps(
        market_data = {underlying_spot, vol_surface,
                       rate_curve},
        state_fields = {barrier_breached, lifecycle_stage})
\end{lstlisting}

\noindent This declaration is consumed by three systems: (1)~the pricing workflow, which reads both market data and the declared state fields before invoking the pricer; (2)~the lifecycle-triggered repricing policy, which uses the declared state fields to determine whether a lifecycle event is material for repricing; (3)~the model risk framework, which audits that all price-relevant inputs are declared and tracked.

%----------------------------------------------------------------------
\subsection{Lifecycle-Triggered Repricing}
\label{sec:lifecycle-repricing}
%----------------------------------------------------------------------

Not every lifecycle event requires an immediate reprice.  A coupon payment changes the dirty price by a known amount; a margin call on a swap \emph{consumes} the latest price but does not produce a new one.  The repricing policy classifies lifecycle events by their effect on the price function:

\begin{definition}[Repricing Classification]
\label{def:repricing-class}
A lifecycle event is classified as \textbf{material} if it changes the output of $P(u, \mathrm{state}_t(u), \mathrm{market\_data}_t)$ for fixed market data.  That is: the event changes unit state in a way that changes the price.
\end{definition}
...
\begin{tabular}{l l l}
\toprule
\textbf{Lifecycle event} & \textbf{Material?} & \textbf{Repricing action} \\
\midrule
Coupon/dividend payment & Yes & Immediate reprice.  Dirty price discontinuity. \\
Option exercise/expiry & Yes & Immediate reprice.  Unit state changes price regime. \\
Futures daily settlement & Yes & Immediate reprice.  Accumulated cost resets. \\
SBL partial return & Yes & Immediate reprice.  Loan quantity changes value. \\
SBL rate change & Yes & Immediate reprice.  Fee rate change affects accrual. \\
Barrier knock-in/out & Yes & Immediate reprice.  Option activation or deactivation. \\
Callable bond call & Yes & Immediate reprice.  Price collapses to call price. \\
\midrule
Mark-to-market margin call & No & No reprice.  The margin call \emph{consumes} the latest price; it does not change the instrument's price. \\
Collateral substitution & No & No reprice for the hedged instrument.  (The collateral pool valuation changes, but that is a separate unit.) \\
Regulatory reporting event & No & No reprice.  Reporting is a read-only projection. \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Implementation.}  The lifecycle workflow's signal to the pricing workflow carries the event classification:
...
LifecycleSignal:
    ...
    unit_id:             UnitId
    timestamp:           Timestamp
    material:            Boolean    -- does this event change P_t?
    invalidates_greeks:  Boolean    -- are cached Greeks now invalid?
    closed_form_adj:     Optional[Decimal]  -- known adjustment amount
\end{lstlisting}

\noindent When \texttt{material = True}, the pricing workflow treats the signal as equivalent to an upstream market data refresh: it drains the signal, reads the post-event unit state, and invokes the pricer immediately (subject to market data freshness).  When \texttt{material = False}, the pricing workflow acknowledges the signal but does not trigger an early reprice.

%----------------------------------------------------------------------
\subsection{Worked Example: Bond Coupon Payment}
\label{sec:worked-coupon}
%----------------------------------------------------------------------

This example traces the full sequence from lifecycle event through repricing, PnL explain, and publication.

\paragraph{Setup.}  A corporate bond (unit: CORP-BOND-001) with:
\begin{itemize}[nosep]
\item Par value: \$100
\item Coupon: 5\% annual, semi-annual payments of \$2.50
\item Day count: 30/360
\item Last coupon date: 6 months ago
\item Yield to maturity: 4.8\%
\end{itemize}

\noindent At 16:00 UTC (pre-coupon), the last FIRM valuation:
\begin{center}
\small
\begin{tabular}{l r}
\toprule
\textbf{Field} & \textbf{Value} \\
\midrule
\texttt{dirty\_price} & \$102.50 \\
\texttt{clean\_price} & \$100.00 \\
\texttt{accrued} & \$2.50 (6 months of accrual at 5\% on \$100 par) \\
\texttt{modified\_duration} & 4.2 \\
\texttt{dv01} & \$0.042 \\
\texttt{quality} & FIRM \\
\bottomrule
\end{tabular}
...
\paragraph{Step 1: Lifecycle event fires (16:05 UTC).}  The bond coupon workflow~\cite[\S14.4.2]{ledger} wakes on the coupon date.  The lifecycle function computes:
\begin{lstlisting}
pending_tx = ComputeCouponPayment(
    unit_id = "CORP-BOND-001",
    coupon_date = "2026-03-28",
    market_data = mkt_snap)
\end{lstlisting}
The executor commits:
\begin{itemize}[nosep]
\item Move: \texttt{Move(from: issuer\_wallet, to: holder\_wallet, unit: USD, quantity: 2.50)}
\item State update: \texttt{coupon\_2026-03-28: "paid"}, accrued interest resets to zero.
\end{itemize}

\paragraph{Step 2: Lifecycle signal sent to pricing workflow.}
\begin{lstlisting}
workflow.SignalExternalWorkflow(
    pricing_workflow_id("CORP-BOND-001"),
    "lifecycle_event_completed",
    {event_type: "COUPON_PAYMENT",
     material: True,
     invalidates_greeks: True,
     closed_form_adj: -2.50})
\end{lstlisting}

\paragraph{Step 3: Taylor approximation invalidated.}  The approximation service receives the invalidation.  Between 16:05 and the full reprice, any query for CORP-BOND-001 returns:
\begin{itemize}[nosep]
\item \texttt{dirty\_price} = \$102.50 $-$ \$2.50 = \$100.00 (closed-form adjustment)
\item \texttt{quality} = APPROXIMATE (adjusted, not FIRM)
\end{itemize}

\paragraph{Step 4: Pricing workflow fires (16:06 UTC).}  The pricing workflow receives the lifecycle signal and fresh market data (yield curve has moved by $+2$~bp since 16:00).  It reads the post-coupon unit state:
\begin{itemize}[nosep]
\item Coupon 2026-03-28: paid.  Accrued interest since payment: 1 day = $\$100 \times 5\% \times \frac{1}{360} = \$0.014$.
\end{itemize}

\noindent The pricer computes with post-coupon state and fresh market data (yield now 4.82\%):
...
\textbf{Field} & \textbf{Value} \\
\midrule
\texttt{dirty\_price} & \$99.93 \\
\texttt{clean\_price} & \$99.92 \\
\texttt{accrued} & \$0.014 (1 day of fresh accrual) \\
\texttt{modified\_duration} & 4.2 \\
\texttt{dv01} & \$0.042 \\
\texttt{quality} & FIRM (pending PnL explain) \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Step 5: PnL explain.}  The explain must account for the coupon payment as a cashflow:

\begin{align*}
\text{Total price PnL} &= 99.93 - 102.50 = -\$2.57 \\[6pt]
\text{Coupon cashflow} &= -\$2.50 \quad\text{(dirty price drops by coupon)} \\
\text{Theta (1 day accrual)} &= +\$0.014 \\
\text{Duration (2 bp yield rise)} &= -4.2 \times 0.0002 \times 100.00 = -\$0.084 \\[6pt]
\text{Explained} &= -2.50 + 0.014 + (-0.084) = -\$2.57 \\
\text{Unexplained} &= -2.57 - (-2.57) = \$0.00 \quad\checkmark\;\textsc{Pass}
\end{align*}

\noindent The ValuationRecord is published with \texttt{quality = FIRM}.  The coupon cashflow appears explicitly in the PnL explain breakdown.  Without the lifecycle-valuation ordering protocol, the coupon would appear as $-\$2.50$ of unexplained PnL---a certain \textsc{Fail}.

\paragraph{Key observation.}  The lifecycle event is not ``noise'' in the PnL explain---it is a first-class term.  The PnL explain function (Definition~\ref{def:pnl-explain}) already accepts a \texttt{cashflows} parameter: ``\texttt{sum(cf.amount for cf in cashflows)}''.  The coupon payment is a cashflow from the lifecycle engine.  The consistency requirement is that this cashflow reaches the PnL explain function at the same time as the new ValuationRecord.  The ordering protocol guarantees this.

%----------------------------------------------------------------------
\subsection{Worked Example: SBL Fee Accrual Across Valuation Cycles}
\label{sec:worked-sbl}
%----------------------------------------------------------------------

This example shows how the SBL loan unit's value changes between valuation cycles due to fee accrual---a case where the Taylor approximation works correctly because fee accrual is continuous, not discontinuous.

\paragraph{Setup.}  An SBL loan unit (LOAN-SBL-001):
\begin{itemize}[nosep]
\item Lender: Entity A.  Borrower: Entity B.
\item Underlying: 10{,}000 shares of NVDA, market value \$9{,}125{,}000.
\item Fee rate: 60 bp/year (borrower pays lender).
\item Daily fee: $\$9{,}125{,}000 \times 0.0060 / 360 = \$152.08$.
\item The ``loan unit'' is valued from the lender's perspective: the value represents the present value of accrued and future fee income.
\end{itemize}

\noindent At 16:00 UTC (Day 0), the last FIRM valuation of the loan unit:
\begin{center}
\small
\begin{tabular}{l r}
\toprule
\textbf{Field} & \textbf{Value} \\
\midrule
\texttt{dirty\_price} (accrued fee receivable) & \$350.00 \\
$\Theta$ (fee accrual rate, per day) & \$152.08 \\
\texttt{quality} & FIRM \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Day 1: Between cycles (approximate pricing).}  One day passes.  The approximation service computes:
\begin{align*}
P_{\mathrm{approx}}(\text{LOAN-SBL-001}) &= 350.00 + \Theta \times \Delta t \\
&= 350.00 + 152.08 \times 1 \\
&= \$502.08 \quad\text{(quality = APPROXIMATE)}
\end{align*}

\noindent The theta term for the SBL loan unit IS the fee accrual rate.  The Taylor approximation is exact for linear accrual (no higher-order terms).  No lifecycle event has fired---fee accrual is a continuous process computed from unit state (fee rate, loan quantity) and elapsed time.

\paragraph{Day 1: Full reprice (16:00 UTC).}  The pricer reads the loan unit state (fee rate unchanged, quantity unchanged, NVDA market value now \$9{,}140{,}000 after a small price move) and computes:
\begin{center}
\small
\begin{tabular}{l r}
\toprule
\textbf{Field} & \textbf{Value} \\
\midrule
\texttt{dirty\_price} (accrued fee receivable) & \$502.50 \\
$\Theta$ (updated for new NVDA price: $9{,}140{,}000 \times 0.0060/360$) & \$152.33 \\
\texttt{quality} & FIRM \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{PnL explain.}
\begin{align*}
\text{Total PnL} &= 502.50 - 350.00 = +\$152.50 \\
\text{Theta (1 day at old rate)} &= +\$152.08 \\
\text{Delta (NVDA price sensitivity of fee base)} &= +\$0.42 \\[4pt]
\text{Explained} &= 152.08 + 0.42 = +\$152.50 \\
\text{Unexplained} &= 152.50 - 152.50 = \$0.00 \quad\checkmark\;\textsc{Pass}
\end{align*}

\noindent For SBL loans, theta is the dominant driver of PnL.  The Taylor approximation is accurate because fee accrual is linear in time.  No lifecycle event fired (the fee accrual happens continuously within the unit state's parameters), so no Taylor invalidation was needed.

\paragraph{Contrast with partial return.}  If the borrower returns 5{,}000 shares mid-day (a lifecycle event), the loan quantity changes, the fee base halves, and the accrual rate drops.  This IS a lifecycle event that triggers immediate repricing: the old theta (\$152.08/day) is invalid after the return because the loan is now half the size.  The lifecycle signal carries \texttt{material = True}, \texttt{invalidates\_greeks = True}, and the pricing workflow fires immediately with the post-return unit state.
...
\item (v0.2) Consistency with the lifecycle engine (Section~\ref{sec:lifecycle-consistency}): the formalisation of the price function's dependence on unit state ($P_t(u) = P(u, \mathrm{state}_t(u), \mathrm{market\_data}_t)$), the lifecycle-before-valuation ordering protocol, Taylor approximation invalidation on lifecycle events, state-dependency edges in the extended Pricing DAG, lifecycle-triggered repricing classification, and two worked examples demonstrating the complete lifecycle-valuation-explain pipeline.
\end{enumerate}
...
\paragraph{Lifecycle event fan-out for structured products.}  When a lifecycle event on an underlying instrument (e.g., coupon payment on a bond embedded in a structured note) changes the underlying's price, the structured note's pricing workflow must also be notified.  The current DAG signalling handles this for market data changes (the bond's new FIRM price signals downstream dependents), but the lifecycle signal itself is unit-local.  The question is whether the structured note needs its own lifecycle-triggered reprice (because the underlying's state change is material to the note's price) or whether the normal DAG cascade---underlying reprices, signals downstream, downstream reprices---is sufficient.  For most cases, the cascade is sufficient; the open problem is identifying cases where it is not (e.g., a structured note whose payoff depends on whether the underlying bond has been called, not just on the bond's price).

\paragraph{Cross-lifecycle-event ordering.}  Section~\ref{sec:ordering-protocol} specifies that lifecycle events for a unit must complete before that unit's next valuation cycle.  But when multiple units have correlated lifecycle events on the same date (e.g., multiple bonds paying coupons on the same day), the order in which those lifecycle events are processed may affect portfolio-level PnL attribution.  The per-unit ordering guarantee is sufficient for per-unit PnL explain; the portfolio-level attribution under simultaneous lifecycle events is an open problem.
