...
The framework needs a canonical vocabulary for products and lifecycle events: one normalised description of what is traded, one of how obligations evolve, and one deterministic rule for when a transition is permitted. The ISDA Common Domain Model (CDM), governed by FINOS, supplies all three. The ledger adopts it as its domain model, deriving the correspondence from the framework's own primitives---wallet, unit, move, transaction, conservation---and then storing each CDM record intact alongside the transaction it generates.
