...
A listed future exercises every primitive at once. An equity trade is one transaction --- cash one
...
conservation primitives of \S\ref{sec:ledger} and the three-home state model of \S\ref{sec:sec04}.
...
%==============================================================================
\subsection{Primitives recalled, and where the future's state lives}
%==============================================================================

A \emph{wallet} $w$ holds a signed quantity $h(w,u)$ of a \emph{unit} $u$; a \emph{move} transfers a
positive quantity of one unit from a source wallet to a destination ($w_s(u)\mathrel{-}=q$,
$w_d(u)\mathrel{+}=q$); \emph{conservation} is $\sum_w h(w,u)=0$ for every $u$, an algebraic
consequence of the move pattern, not a runtime check. Write $\netq(w,u):=h(w,u)$.

State attaches to three homes (\S\ref{sec:sec04}):

\begin{center}\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Map} & \textbf{Key} & \textbf{Discipline} \\
\midrule
\emph{ProductTerms}  & $u$     & immutable, append-only, versioned; registration-total \\
\emph{UnitStatus}    & $u$     & materialised projection of the log; registration-total \\
\emph{PositionState} & $(w,u)$ & monotone carrier; \code{Option} accessor \\
\bottomrule
\end{tabular}
\end{center}

\begin{principle}[UnitStatus is a projection, not a source of truth]
UnitStatus holds one value per unit, shared---read identically by every holder. Its value changes
over time, but UnitStatus is not a separate source of truth: the immutable event log is. Every change
is caused by a logged event; the stored value is overwritten in place only as a read cache. Replaying
the events up to any point rebuilds the exact value that held then, so nothing is lost by overwriting
and there is no other way to change the value. A value exists from registration onward.
\end{principle}
...
enforced one level up at the event handler (C2), since a refinement type on a sum of exact integers
is free in no production language.
...
Both items are carried to the consolidated register in \S\ref{sec:conclusion}.
...
must be shown to preserve C2 and C11. The cost stands. \emph{Owner: the performance/risk agent.}
