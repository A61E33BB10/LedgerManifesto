...
-- =============================================================================
-- Ledger.hs  --  The Ledger v11.0, consolidated reference implementation.
--
...
-- magnitude mQty is positive. Because a Move names BOTH parties, it conserves by
...
-- | The sole ingest path: positivity is a parse-boundary discipline, not a type
--   guarantee (Qty must also host negative balances), so `move` rejects a
--   non-positive magnitude. Total.
move :: WalletId -> WalletId -> UnitId -> Qty -> Timestamp -> SourceId -> Maybe Move
move ws wd u q t s
  | q > mempty = Just (Move ws wd u q t s)
  | otherwise  = Nothing
...
contribution m = Balances (Map.fromListWith (<>)
  [ ((mTo   m, mUnit m),      mQty m)
  , ((mFrom m, mUnit m), qneg (mQty m)) ])
...
moveDelta (Move s d _ q _ _) = [(s, qneg q), (d, q)]

-- | Mandate issuance: a real two-named-wallet transfer of u_MA, manager -> client.
issueMandate :: WalletId -> WalletId -> UnitId -> Move
issueMandate manager client uMA = Move manager client uMA (Qty 1) (Timestamp 0) (SourceId "issuance")
...
crystallise (Qty x) from to
  | x > 0     = Just (Move from to usd (Qty x)          (Timestamp 0) (SourceId "fee"))
  | x < 0     = Just (Move to   from usd (Qty (negate x)) (Timestamp 0) (SourceId "fee"))
  | otherwise = Nothing
...
value (PriceVec p) l =
  foldMap (\((_, u), ps) -> markValue (psBalance ps) (p u)) (Map.toList (ledgerPS l))

pnl :: PriceVec -> Ledger -> PriceVec -> Ledger -> Cash
pnl pv0 l0 pv1 l1 = value pv1 l1 `cashSub` value pv0 l0
