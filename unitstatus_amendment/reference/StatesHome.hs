...
data StatusWrite
  = SetLifecycle    Lifecycle  -- lifecycle event: Listed -> Active -> Expired -> Closed
  | SetLastSettle   Qty        -- settle event: the marked settlement price (a logged observation)
  | SetSupersededBy UnitId     -- Breaking amendment (C8): point the old unit at its successor
  deriving (Eq, Show)
...
applyDelta (ValidDelta sd) l
  ...
  | otherwise = Right l
      ...
      , ledgerUS = if null (sdStatus sd)
                     then ledgerUS l
                     else Map.adjust (\s -> foldl' (flip applyStatus) s (sdStatus sd)) u (ledgerUS l)
                          -- overwrite-in-place as a read cache, one field per StatusWrite;
                          -- u is registered, so adjust HITS and can only REPLACE field values --
                          -- it cannot fabricate a half-registered unit, and there is no other
                          -- door to a UnitStatus field than this fold of `applyStatus`.
            ...
            Right ( Superseded u uFresh
                  , l { ledgerPT = Map.insert uFresh (ProductTerms (tvNew :| [])) (ledgerPT l)
                      , ledgerUS = Map.insert uFresh defaultStatus
                                 $ Map.adjust (applyStatus (SetSupersededBy uFresh)) u
                                              (ledgerUS l)
                      } )
...
--      ties a write to a HANDLER (a settle handler cannot author a hwm write);
...
main = do
  ...
  let ghostSD = StateDelta (UnitId "GHOST") Map.empty [SetLifecycle Listed] Nothing
