...
itself trades futures, and an instrument listed but never traded. Unit state therefore
...
\subsection{An Instrument Listed but Not Yet Traded}
\label{sec:untraded}
...
product-declared defaults (e.g.\ \texttt{lifecycle\_stage = LISTED},
...
An untraded option moves $\texttt{LISTED}\to\texttt{ACTIVE}\to\texttt{EXPIRED}$ entirely
...
\item Lifecycle guards ($\texttt{LISTED}\to\texttt{ACTIVE}\to\texttt{EXPIRED}$):
...
$\texttt{EXPIRED}\to\texttt{LISTED}$) is enforced by these tests, not by types --- distinct
