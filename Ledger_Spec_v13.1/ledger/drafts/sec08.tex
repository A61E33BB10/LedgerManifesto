...
A lifecycle event is computed from the present, not from history. It reads the unit, that unit's current state, and the observed market data. It returns the moves to make together with the next state. It commits nothing.
...
It mutates nothing and reads no hidden global. It returns a description: a list of wallet-to-wallet moves and the next state. The function is local to the unit and the event; it requires no knowledge of other units, wallets, or pending events. \texttt{applyTx} commits its output atomically.

Lifecycle is therefore state-transition logic plus a collection of moves. The definition presupposes no pricing model. This generality is required. The ledger carries convertible bonds, margin loans with bespoke collateral rules, and rule-based strategies whose economics live in state transitions rather than closed-form prices. The framework demands only the transition function above, so each is handled at the lifecycle level on the same footing as a plain-vanilla instrument.

\noindent In code $f$ is \texttt{handle}. It reads the ledger, which supplies $\text{state}_t(u)$. It takes the event, which carries the unit and any market datum. It returns the proposal as a \texttt{Transaction}: the moves and the state delta they carry. It never returns a changed ledger.
...
\begin{mentalmodel}
Picture the coupon recorded in a notebook that is only ever added to: the line ``coupon at date $T$: paid'' is already on the page. Running the event again reads that line, finds the payment already entered, and writes nothing. The second pass is empty because the page shows the work done, not because a separate list remembers the event was seen---strike the line and the same event pays the coupon once more, since the emptiness lives in the state read, never in a memory of past attempts.
\end{mentalmodel}
...
One function changes the ledger; nothing else does. A lifecycle function only proposes a transaction. The proposal becomes part of the ledger only when this one function commits it.
...
The Transaction returned by a lifecycle function is a proposal. It takes effect only when \texttt{applyTx} commits it. Conservation is not checked at this boundary; it is a property of the Transaction type. The signed edge-sum of the moves is \texttt{mempty} by construction (\S\ref{sec:ledger}, C2/C9), so there is no unconserved Transaction to reject. \texttt{applyTx} requires that the unit be registered. It inserts or updates rows and never deletes one. It commits the whole Transaction atomically to the immutable event log of \S\ref{sec:ledger}. Product guards (sufficient source balance, stage legality) live in \texttt{handle}, which builds the proposal.

Confining mutation to one door has three consequences. First, conservation holds in exactly one place: the Transaction type. A defective contract therefore cannot break an invariant: there is no unconserved proposal for \texttt{applyTx} to admit, and a balance moves only via a conserving \texttt{Move}. What it can still do is book wrong economics --- a conserving transaction whose amounts disagree with the declared terms --- and that error is detected by recomputation from declared data (\S\ref{sec:processor-contract}). Second, lifecycle functions stay pure: they compute a proposal and cannot alter state, so they are testable, composable, and replaceable. Third, the audit trail is complete: every state change passes through \texttt{applyTx} and is logged. The attack surface for state corruption is bounded to this single door, whose properties P1--P10 are stated and tested in \S\ref{sec:invariants}. Economic correctness of what the door admits is governed by \S\ref{sec:processor-contract}. The role is that of an operating-system kernel: all privileged operations pass through one narrow, auditable interface.

\noindent In code the step is the two halves in sequence: \texttt{handle} builds the proposal and \texttt{applyTx} commits it. Replay folds this step over the event stream.
...
\noindent Replay is a \emph{fold homomorphism}: replaying two event batches joined, \texttt{replay (xs <> ys)}, equals replaying each in turn, \texttt{replay xs >=> replay ys}. Here \texttt{f >=> g} runs \texttt{f}, feeds its result to \texttt{g}, and stops at the first error. The reconstructed ledger is therefore independent of where a checkpoint is cut. Checkpoint-independence (\S\ref{sec:invariants}, P8) is a consequence of the law, not a separate test. Determinism follows from \texttt{applyTx} being pure and total.

\noindent The event log is therefore canonical. Every state change passes through the single door and is logged. Replay rebuilds any state from those events alone. Any ledger, current or historical, is reproducible by replaying its log. The projections of \S\ref{sec:states}, \texttt{UnitStatus} among them, are rebuilt this way.

\subsection{Purity}

A lifecycle function computes; it does not observe its surroundings. Run it now or in a later replay, on any machine: the same inputs give the same outputs.
...
\begin{mentalmodel}
The ledger's history is a notebook written only by adding lines, never by erasing them. To see any past date, cover every line entered after it and re-total the page from the top: the holdings that stood at that date reappear exactly, because the earlier lines were never overwritten. No daily snapshot is stored and read back---each past total is recomputed from the surviving lines, which is why the reconstruction is exact only for a record that appends and never erases.
\end{mentalmodel}

Time travel reconstructs the exact ledger state at any historical timestamp $t$. This is not merely today's totals replayed: it is what each position was, what it referenced, and how its value evolved. Four situations make this non-trivial. Any implementation must reconstruct each.
...
\item \textbf{Corporate actions and share denominators.} A 2-for-1 split turns 100 pre-split shares at a pre-split price into 200 post-split shares at roughly half the price; economic value is preserved but the unit changes. Replaying trade tickets against the current share definition mis-describes the $t_0$ position. Correct time travel reconstructs both views and explains the jump as a corporate action, not a reference-table edit.
...
Unit state is not stored as a private dictionary on the unit. It lives in the three-home state model of \S\ref{sec:states}. There the per-unit lifecycle projection is \texttt{UnitStatus}: a read cache of the event log, rebuilt exactly by replay (\S\ref{sec:states-unitstatus}).

\noindent A lifecycle function never writes \texttt{UnitStatus} off-log. It returns a Transaction; \texttt{applyTx} logs the event; the projection is rebuilt from the log. This is why time travel is exact: \texttt{clone\_at($t$)} rebuilds the projection from the events up to $t$.
...
\begin{enumerate}[nosep]
\item \textbf{Valuation justification.} Current valuation is explained directly from the state, without chasing flags across systems: remaining cash flows, exercised versus unexercised rights, triggered versus untriggered features.
...
Every lifecycle function category is either a pure function or a deterministic projection, and each is covered by named properties (\S\ref{sec:invariants}). Contract evaluation maps (input, state, conditions) to moves, pure (\S\ref{sec:contracts}), covered by P1 and the totality oracle. State transition maps (unit, state, event) to (moves, new state), pure (Principle~\ref{prin:purity}), covered by P5 and P6 and the valid-transitions oracle. Transaction commit, \texttt{applyTx}, is the sole door --- registration check, monotone atomic commit --- deterministic and the sole mutator, covered by P1 (conservation by construction) and P2 (atomic commitment); no function bypasses it. Views and snapshots are read-only accessors, covered by P8 (snapshot and time-travel consistency). The PnL projection $V_t = \sum w_t(u) \cdot P_t(u)$ is pure, derived from state and prices, covered by P10 (path-independence). Balance computation, $w_t(u)$ from move-stream replay, is a pure projection of the log, covered by P1 and P3 (referential integrity).
...
At each rebalance or leverage reset, the lifecycle function computes the trades required to hold the target allocation: moves between the strategy wallet and market wallets. It also computes an updated state recording new weights, reference level, and performance lock-in. Subscriptions and redemptions are lifecycle events. A subscription creates units, triggers the underlying rebalance, and updates composition and leverage. A redemption destroys units and unwinds correspondingly.
