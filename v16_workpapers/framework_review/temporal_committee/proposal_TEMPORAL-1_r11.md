# Temporal Committee — Proposal TEMPORAL-1, Round 11 (FINAL consensus candidate)

**Stance.** The assembly on **three record kinds + one lineage discipline**; all seven red-team scenarios
(S1–S7) hold. FORMALIS signed CONSENSUS YES; this round applies TuringAward's single blocking close — the
missing harvest row `prop_noSilentUnderAdmit` (the injectivity dual of exactly-once) — plus FORMALIS's
three witness-obligation folds, all **witness/harvest-only, reopening no design decision**. Three kinds:
**(1) projection** (recompute-on-read, never stale); **(2) re-entered observation** (model output;
read-back unconditional; stale when a consumed input moves, MD-8); **(3) recorded decision, pinned
as-known** (a verdict recorded with the event — the base ledger's own door admit/refuse, spec l.1051; the
MD-16 gate verdict is the same shape). Lineage: cause-derived `txid` over recorded inputs, pinned **cut**,
forward-only refold. Settled: **Fork A** single-transaction write; **A′** FLAG; **Fork B**
...
(S6): strip Temporal, rebuild chain + derived states + gate verdicts from the log (R-02).
...
The **complete canonical harvest** — 11 properties covering S1–S7, COVERAGE-β, and **both halves of I2**
(`prop_exactlyOnceAdmission` + `prop_noSilentUnderAdmit`); a narrower list is not canonical. Each must be
shown to **fire** (a precondition never generated = a §3 defect):
...
| `prop_exactlyOnceAdmission` (S4, double-admit half of I2) | N redeliveries × W workers × door crash-restart, same txid | exactly one row per txid; none starved |
| `prop_noSilentUnderAdmit` (S4, injectivity/under-admit half of I2) | two **fine-grained-distinct** causes with near-identical input-cuts (a too-coarse cut label would collapse them to one txid) | distinct causes → **two distinct** admitted txids; never silently under-admitted (the exact-grained-input-cut guard, I2) |
| `prop_gateStateAtomic` (S7) | failover injected at each of gate1/gate2/construct/propose | whole {state,verdict} pair or nothing |
| `prop_sandwichCANInvariant` (S3) | CAN injected in **both** positions: after a leg's emission is durable, **and** mid-compute of a kind-2 leg (not-yet-durable) | pre-admitted leg reproduces **byte-identical by read-back** of its durable emission; a re-driven leg is S4-idempotent — **no half, no double, value β-bounded** (not byte-identical) |
| `prop_wipeRebuildEqualsLog` + `prop_fabricatedTxidRefusedOrAudited` (S6) | forge/poison the cache, replay, wipe, rebuild; **two fabrication generators** — (i) novel/inconsistent txid, (ii) **structurally-valid, txid-consistent, value-corrupted** (real logged cause, wrong value) | rebuilt = clean rebuild; (i) door-refused, (ii) **audit-caught (D7)** — so the audit branch actually fires, never silent |
...
| `prop_clockSkewInvariant` (S5) | adversarial per-worker/DC wall-clock offsets over a history including a **commuting same-execution-time pair** | {admitted set, fold result, execution+monitor times} invariant; door-order invariant **up to commuting same-execution-time transposition**; a skewed timer fire yields the identical txid; skew never changes a recorded time by type |
...
- **Parking test exercised, empty.** Derived stream as a "second store" (no — tagged sub-stream,
  C-2.8/12.5); gate verdict vs recompute-on-read (C-4.11 — a pinned event-outcome). Must **not** turn on
  the Valuation Manifesto's **PARK-1**, which MD-16's mechanism neither reopens nor turns on.
...
- **Load model (biggest unknown).** Volume per unit/underlying/day sizes `K`, the door pool, derived
  ...
  coupling) gate on it, non-correctness; do not force. Gate-2 "undecidable" parks by design — a flagged item.
