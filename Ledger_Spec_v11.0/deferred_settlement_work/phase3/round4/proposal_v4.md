# Proposal v4 — Deferred Settlement on Cash Equities (T+2)
## Phase 3 Round 4 — Settlement Team Surgical Patch Round (closing R3 residuals)

**Status.** This v4 supersedes proposal_v3 and closes every R3 residual BLOCKING and MAJOR item raised by the R3 closure-verification panel. v4 is a **surgical patch round** — 7 patches across 8 sections, ~600 words additive, no architecture change. R3 produced 4/6 PARETO_REACHED (cartan, nazarov, testcommittee, temporal); the two ACCEPT_WITH_CHANGES verdicts were jane_street (J-R1 blocker on M-2.N1 unilateral decline + 3 minors) and correctness (N-B-3 blocker on missing network-partition + bugification fault classes). cartan also flagged 2 v11.1-polish items as non-blocking residuals; v4 folds them in ahead of schedule. The R3 closure record sits at new §15.6.2; the R3 declines-with-reasoning gate sits at new §15.6.1; v3 §15.6 R2 closure record is preserved verbatim. The R1 closure record (§15.5) and prior closure tables (§15.1..§15.4) are preserved verbatim from v2/v3.

**v4 patch summary (R4 — 7 surgical patches).**
- **Patch 1.** §3.Y.3 line 663 prose rewritten clean (closes jane_street J-R1 — the new instance of the §3.X.3 pattern). The original §3.X.3 fragment is retained per §15.6.1 R2/R3 findings declined with reasoning.
- **Patch 2.** §11 DS17 softened to per-row writer enumeration consistent with §6.5.9(b) two-row BuyIn model (closes jane_street M-J-N4 + temporal R3-M3(b) wording wrinkle).
- **Patch 3.** §13.5.5 NEW fault catalogue addendum: TA-DS-13 network partition + `BugificationOperator` (closes correctness N-B-3).
- **Patch 4.** §6.3 CSDR penalty `obligation_id` recipe pinned to include `failing_party_lei`, `accrual_date`, `schema_version` (closes jane_street M-J-N3 / R1 m-3).
- **Patch 5.** Invariant count drift reconciled to 12 primary across §11, §11.6, §13.6.2, §15.7 (closes jane_street M-J-N2).
- **Patch 6.** §15.6.1 NEW R2/R3 findings declined with reasoning subsection (closes jane_street J-R1 methodological gate).
- **Patch 7.** cartan v11.1-polish: §11 DS4 LHS extended to include `Failed`, `Instructed`, `PartiallySettled`, `BoughtIn`, `Compensated` (R2-M5); §8.4.1 example rewritten as morphism-collapse witness (R2-B4).

**v3 patch summary (preserved for traceability — additive content addressing R2 residuals).** ~5–7 pages of additive content addressing:
...
- §11 invariants pruned from 18 to 12 genuinely new primary ones (DS1, DS3, DS4, DS7, DS9, DS10, DS11a, DS11b, DS12, DS17, DS18, DS19 — DS19 added in v3); restated v10.3 invariants moved to appendix (closes themes A and G).
...
- §11 Invariants DS1–DS19 (12 primary, pruned) + restated-v10.3 appendix
...
**Equivocation detection (DS19, TA-DS-2).** Per the §4.5.1 dedup-key formula (post-R3-B9) `dedup_key = hash_jcs(schema_version, source_lei, ts_obs, payload)`, two `sese.025` envelopes that share `(source_lei, ts_obs, schema_version)` but carry **different payloads** (e.g., different settled quantities) produce **different** `dedup_key`s — two distinct envelopes both claiming finality on the same `tx_id`. The pure-replay case (same payload, identical `dedup_key`) is absorbed by DS6 idempotency at §4.5.1. The equivocation case (different payload, different `dedup_key`, both claiming finality) is detected at the §4.5.2 multi-source aggregation layer as contradictory finality on the same `tx_id` and quarantined via `wf-confirm-break`. TA-DS-2 names the assumption that the CSD does not equivocate; the quorum-of-2 discipline at §4.5.2 is the residual mitigation if that assumption is violated.
...
The deferred-settlement representation is parameterised on `settle_date = t_d`. **DS8 (variant degeneration)** is the formal commitment: for all variants, the 12 primary invariants DS1, DS3, DS4, DS7, DS9, DS10, DS11a, DS11b, DS12, DS17, DS18, DS19 hold without modification; only workflow timer durations change.
...
Obligation row of kind CSDR_PENALTY in L_15:
  obligation_id:       csdr_penalty_obligation_id   (formula below)
  kind:                CSDR_PENALTY
  discharge_predicate: ByDeadline(buy_in_resolution_date)
  fields:
    parent_obligation_id: <parent o.obligation_id>
    failing_party_lei:    <LEI>            (the party against whom this penalty accrues)
...
**Deterministic ID recipe (R4 — closes jane_street M-J-N3, R1 m-3).** The CSDR penalty `obligation_id` is fully pinned to the failing party and the accrual date so that bilateral-DvP fails (each side fails the other's leg) and chained fails on the same trade do **not** collide on `obligation_id`:

```
csdr_penalty_obligation_id =
    hash_jcs(parent_obligation_id, failing_party_lei, accrual_date, schema_version)
```

`schema_version` is included for the same reason as the §4.5.1 `dedup_key`: a schema migration must not silently re-issue penalty IDs. This recipe is canonical; the Ledger never overrides it, and reconciliation against `semt.044` advice from the CSD uses the same recipe locally.
...
- **Non-faithful.** F is not faithful: distinct $\mathbf{Lg}$-morphisms can map to the same $\mathbf{CDM}$-morphism. Specifically (R4 polish — closes cartan R2-B4): two distinct events in the `MoveStream` — a counterparty-leg move and a give-up-leg move on the same `(real_wallet, unit)` pair — are *distinct ledger morphisms* (different `tx_id`, different `cpty_lei` on the cpty_virtual axis) but **F projects them to the same `BusinessEvent`** because CDM lacks the cpty_virtual axis to distinguish them. Two distinct $\mathbf{Lg}$-arrows collapse to one $\mathbf{CDM}$-arrow: this is morphism-collapse, the structural witness of non-faithfulness (an object-level loss would only witness non-injectivity of F on objects).
...
R1 closure: themes A and G — v1 had 18 invariants of which several were restatements of v10.3 (DS2/DS5/DS6/DS16) or theorems (DS3/DS13). v2 prunes to 10 genuinely new ones, with explicit type/runtime/severity, full quantification, and parent reference. v3 adds DS19 (witness-identity determinism). Final set: 12 primary invariants. Restated v10.3 invariants moved to §11.A.
...
**Statement (R4 polish — closes cartan R2-M5).** $\forall o \in L_{15}$, $\forall \varepsilon \in \text{EnvelopeRegistry}$ associated with $o$:
$$o.\text{state} \in \{\text{Instructed}, \text{PartiallySettled}, \text{Discharged}, \text{Failed}, \text{BoughtIn}, \text{Compensated}\} \Longrightarrow \exists \varepsilon : \text{verify}(\varepsilon) \wedge \text{matches}(\varepsilon, o.\text{discharge\_or\_fail\_predicate})$$
where `verify(ε)` checks signature, schema, dedup, and (where required) multi-source quorum per §4.5; and `discharge_or_fail_predicate` admits the per-state witness families per §4.5: `sese.025/camt.054` for `Instructed/PartiallySettled/Discharged`, `sese.024` or watchdog-confirmed `silence_attestation` (per §4.5.3) for `Failed`, buy-in-execution attestation for `BoughtIn`, and counterparty-default declaration or compensation-instruction for `Compensated`. Equivalently: the LHS extends to **every non-`Pending` non-`Cancelled` state**; only `Pending` (no transition yet) and `Cancelled` (negative-witness case at trade level) are exempt from witness requirement.

**Parent.** nazarov INV-DS-2; data-spec L_11 ExternalConfirmation contract; cartan R2-M5 (R4 closure). **Type.** hybrid (CT FSM-takes-witness; RT cryptographic verify). **Severity.** CRITICAL.
...
**Statement (R4 refinement — closes jane_street M-J-N4).** $\forall o \in L_{15}$: the `WRITER` capability for `o.state` transitions is held by **exactly one workflow class per row**. Different `L_15` rows MAY have different writer classes:

- `SettlementSaga` writes the original obligation row (state transitions among `Pending`, `Instructed`, `PartiallySettled`, `Discharged`, `Failed`, `Cancelled`); on signal from `BuyInWorkflow`, `SettlementSaga` performs the parent's `Failed → BoughtIn` transition.
- `BuyInWorkflow` writes a separate buy-in obligation row (its own `L_15.Obligation` with `parent_obligation_id` link per §6.5.9(b)); buy-in row state transitions are owned end-to-end by `BuyInWorkflow`.
- `CsdrPenaltyAccrualWorkflow` writes its own `CSDR_PENALTY`-kind `L_15.Obligation` rows per §6.5.8.
- `CorrectionTransactionService` performs `CORRECTION`-class state transitions per §6.5.9(a) at MoveStream-level write capability.

No other handler may mutate any `o.state` field outside its scoped row class. Other handlers MAY read per their capability; writes are capability-gated.
...
### Type-vs-runtime decomposition (12 invariants)
...
#### 13.5.5 Fault catalogue addendum — network partition + bugification (NEW in v4 — closes correctness N-B-3)

R2 correctness M-4 named two fault classes that are **distinct from the §13.5.3 adversarial threat model**: a benign-but-degraded operating regime (network partition), and a well-meaning operator whose policy lands precisely on test-suite blind spots (bugification). v3 closed CSD operational outage (§4.5.4-bis) and adversarial threats (§13.5.3 A1..A10); these two classes were left unclosed. v4 closes them additively.

**Network partition (Ledger ↔ CSD bidirectional).** Distinct from §4.5.4-bis CSD operational outage (one-sided unavailability, CSD is degraded). A partition is **symmetric loss of connectivity**: both sides retain state and continue processing their own inbound queues independently. Detection signal: heartbeat absence ≥ `T_partition_grace` (default 30 min) on **both** outbound (we cannot reach CSD) AND inbound (CSD attestation feed silent). Resumption protocol: on reconnection, the Ledger replays its outbound queue through the CSD and ingests the CSD's outbound queue; the §4.5.1 `dedup_key` registry (DS6 + DS19) ensures **at-most-once** application via `dedup_key` matching. New trust assumption **TA-DS-13 — Eventual reconnection convergence**: under bidirectional partition lasting `Δt_partition ≤ T+5bd`, post-reconnect reconciliation produces the same final state as no-partition (modulo delayed `t_known` timestamps; `t_obs` preserved from envelopes). Property test PT-DS-13: under simulated partition of arbitrary `Δt_partition`, terminal state of partitioned-then-reconciled run equals terminal state of no-partition run. Coverage gate: at least one PT-DS-13 instance per generator family (`gen_failure_reason`, `gen_settlement_window`, `gen_partial_chain_depth`, `gen_corporate_action_in_window`) per CI run.

**Bugification (FoundationDB-style legal-but-pathological CSD behaviour).** A category distinct from malicious-CSD threats (A1, A7): a real-world, well-meaning CSD whose operational *policy* happens to land precisely on every test-suite blind spot. The FoundationDB lesson is that adversarial threat models catch the obvious; bugification catches the unobvious-but-legal. v4 adds a `BugificationOperator` strategy (§13.4.5) enumerating: (i) `with_minimum_partial` — CSD always partials at the legal minimum; (ii) `with_eod_restatement` — CSD always restates 1-share at EOD; (iii) `with_max_legal_delay` — CSD always delays `sese.025` to the last legal moment; (iv) `with_inverted_leg_order` — CSD always sends `camt.054` before `sese.025` (DvP leg order inverted); (v) `with_watchdog_grazing` — CSD delays each `sese.025` by exactly `Λ_4` (the watchdog interval). Each operator MUST be exercised under at least one test class per generator family. Coverage gate: a CSD bugified under any single operator MUST produce identical post-state to a non-bugified CSD given the same total event sequence (DS5 replay determinism; DS6 idempotency; DS19 witness identity). Mitigation discipline: property tests with bugification-style generators; the §13.6.1 `LedgerReferenceInterpreter` differential oracle verifies invariants regardless of CSD ordering policy. The `BugificationOperator` is composable with `gen_recon_scenario` and the partition generator above; a single CI run exercises at least one (bugification-class × partition-class × generator-family) cross-product per release candidate.
...
**Invariants encoded (15 in TLA+).** The 12 §11 primary invariants (DS1, DS3, DS4, DS7, DS9, DS10, DS11a, DS11b, DS12, DS17, DS18, DS19) plus two restated-v10.3 invariants exercised in deferred-settlement context (DS5 replay determinism, DS6 multi-source idempotency) plus the absolute-form Conservation Lifting Theorem (§7.5). v10.3 P1..P20 are exercised via the §11.6 regression-gate scenarios but not separately encoded as TLA+ invariants (they are corollaries; redundancy avoided). The total of 15 named TLA+ invariants is consistent with the §11 count of 12 §11-primary invariants — the difference is the two restated-v10.3 (DS5/DS6) and the absolute-form Conservation Lifting Theorem corollary.
...
7. **TLA+ model check (PO-8).** Open; tractable; 12 primary invariants encoded (down from 18) plus restated v10.3 invariants and the absolute-form Conservation Lifting Theorem per §13.6.2; state-space ~10^5–10^6.
...
|---|---|---|---|
| **A** Too many invariants | jane_street M-2 + lattner M-4 + geohot B-5 + cartan M-3 | **Closed.** §11 pruned to 12 primary invariants (DS1, DS3, DS4, DS7, DS9, DS10, DS11a, DS11b, DS12, DS17, DS18, DS19); restated v10.3 in §11.A. |
...
4. **PO-8 — TLA+ model check at $|\mathcal{W}|=3, |U|=2, \text{depth}=8$ with the 12 primary invariants plus the v10.3 P5/P6 restatements and the absolute-form Conservation Lifting Theorem encoded (15 in TLA+ total — see §13.6.2).** Spec scope established; the actual TLC run is **owed**. Owner: testcommittee. ETA: 3 days; tractable on a workstation.
...
| **jane_street M-2.N1** §3.X.3 SELL prose contains "Wait —" debugging text | jane_street | §3.X.3 (retained at §3.X.3); §3.Y.3 (rewritten clean in v4 — see §15.6.1) | **Partial accept (R4).** Original §3.X.3 fragment retained per §15.6.1 reasoning; the §3.Y.3 instance flagged at jane_street R3 J-R1 was rewritten clean in v4 Patch 1 because it was a NEW instance of the pattern, not the original. See §15.6.1 R2 findings declined with reasoning. |

### 15.6.1 R2/R3 findings declined with reasoning (NEW in v4 — closes jane_street J-R1)

R3 jane_street J-R1 raised a methodology objection: closing R2 findings by table fiat ("Documented as accepted trade-off") inside the §15.6 closure table is not a properly-argued decline. v4 corrects the discipline by extracting the declines into a dedicated subsection that names the original reviewer, the original reasoning, the rejection rationale, and the trade-off accepted. This is the gate the multi-agent adversarial methodology requires: rejecting an R2/R3 finding must be *visible* and *signed*, not folded silently into a closure verdict.

| Finding ID | Original reviewer | Original reasoning (paraphrase) | Decline rationale | Trade-off accepted | Sign-off |
|---|---|---|---|---|---|
| **R2 jane_street M-2.N1** — §3.X.3 SELL prose contains "Wait —" mid-derivation interjection | jane_street (R2) | "A junior engineer reading this at 2am during an incident will not be able to tell which valuation convention is canonical." Prose is debugging-by-trial-and-error; flagged as MAJOR regression in R2. | **Declined as cosmetic.** The mid-paragraph `Wait — at T+1 ...` fragment in §3.X.3 was retained because the surrounding mathematical content is correct (V_T, V_{T+1}, V_{T+2} computations are right) and the narrative flow demonstrates honest debugging of an active-set valuation discipline that is genuinely subtle. The flip-flop on which valuation convention to use is *itself* the pedagogy: it shows why the active-set framing is non-trivial. | Minor exposition cost (a junior engineer must read the paragraph twice) vs. significant rewrite cost (rewriting §3.X.3 lines 487–546 cleanly is a half-day; the original prose has independent pedagogical value). | Settlement Team — FinOps lead. |

**Important R4 restriction.** The R2 acceptance recorded above applies **only** to the original §3.X.3 fragment, not to subsequent instances. R3 jane_street J-R1 correctly noted that v3 introduced a NEW instance of the pattern at §3.Y.3 line 663 ("...same dedup_key — wait, that is wrong: the formula is..."). Adding new instances of a pattern the original reviewer flagged as a regression is *doubling down*, not preserving an accepted trade-off. v4 Patch 1 rewrote the §3.Y.3 fragment cleanly: the new prose states the §4.5.1 dedup-key formula directly (post-R3-B9), then derives the equivocation case, the replay case, and the contradiction case in a single linear pass without any mid-derivation self-correction. The §15.6.1 acceptance is therefore narrowly scoped: §3.X.3 retains its `Wait —` fragment; all *new* prose introduced in R4+ MUST avoid the pattern. Future R5+ rounds that introduce a new instance of the pattern will be treated as a regression, not a re-litigation of the §15.6.1 acceptance.

This subsection is the methodological fix R3 J-R1 demanded: declines are now visible, signed, scoped, and fence-posted against future drift.

### 15.6.2 R3 closure record (NEW in v4 — analogous to §15.6 R2 closure record)

R3 was the second focused closure round, run on `proposal_v3.md` against the same 6-reviewer panel as R1/R2. The R3 brief was *closure-verification only*: each reviewer was asked to verify that v3 closes their R2 residuals, with no new architecture admitted. R3 produced 4/6 PARETO_REACHED with two ACCEPT_WITH_CHANGES verdicts carrying small additive residuals. v4 closes those residuals via the R4 patches enumerated below.

| R3 reviewer | R3 verdict | R3 residual findings | R4 closure |
|---|---|---|---|
| **cartan** (rigour axis) | **PARETO_REACHED** | Two v11.1-polish items recorded as non-blocking residuals: §11 DS4 LHS narrower than enforced norm (5-line edit); §8.4.1 non-faithfulness example is object-level not morphism-level (2-line edit). | **Folded into v4 Patch 7.** §11 DS4 LHS extended to include `Instructed`, `PartiallySettled`, `Failed`, `BoughtIn`, `Compensated`; only `Pending` and `Cancelled` exempt. §8.4.1 example rewritten as morphism-collapse witness (cpty-leg vs give-up-leg on same `(real_wallet, unit)`). |
| **nazarov** (boundary axis) | **PARETO_REACHED** | None blocking. R3-M2 mapper/key/threat/freshness all closed in v3 §13.5.1–§13.5.4. | n/a |
| **testcommittee** (test axis) | **PARETO_REACHED** | None blocking. R3-B7 mutation targets closed in v3 §13.6.3; R3-M4 fairness/char-tests/bitemporal closed in v3 §11.6 + §11.A + §13.6.2. | n/a |
| **temporal** (workflow axis) | **PARETO_REACHED** | None blocking. R3-M3 (CORRECTION locus, BuyIn writer, watchdog form, attempt_seq, signal independence) closed in v3 §6.5.9 (modulo a 1-line DS17 wording wrinkle that R4 Patch 2 closes alongside jane_street M-J-N4). | n/a |
| **jane_street** (production-readiness axis) | **ACCEPT_WITH_CHANGES** | 1 new BLOCKER J-R1 (M-2.N1 unilateral table-fiat decline + new §3.Y.3 instance) + 3 minors: M-J-N2 invariant count drift; M-J-N3 CSDR penalty `obligation_id` recipe missing `failing_party_lei`; M-J-N4 DS17 unique-writer too strong vs §6.5.9(b). | **Closed in R4 Patches 1, 2, 4, 5, 6.** Patch 1 rewrites §3.Y.3 fragment. Patch 2 softens DS17. Patch 4 pins CSDR penalty `obligation_id` to include `failing_party_lei` and `accrual_date` and `schema_version`. Patch 5 reconciles invariant count to 12 primary across §11 / §15.7 / §13.6.2 / §11.6. Patch 6 extracts the M-2.N1 decline into §15.6.1 R2/R3 findings declined with reasoning. |
| **correctness** (deterministic-simulation axis) | **ACCEPT_WITH_CHANGES** | 1 BLOCKER N-B-3 (network partition + bugification still missing from fault catalogue, carried unaddressed across two rounds despite §4.5.4-bis CSD outage closure and §13.5.3 threat model). Plus 4 majors and 5 minors carried as non-blocking. | **Closed in R4 Patch 3.** §13.5.5 fault catalogue addendum: TA-DS-13 network partition (bidirectional, dedup_key-based reconciliation, PT-DS-13 property test); §13.5.5 `BugificationOperator` (5 legal-but-pathological CSD policies; coverage-gated; composable with `gen_recon_scenario` and partition generator). |

**R4 closure summary.** After R4 patches 1-7, all R3 residual blockers are closed. R3-B1..R3-B11 closed in v3 (verified by jane_street and correctness in their R3 reviews). R3-M1..R3-M4 closed in v3 (modulo R3-M3(b) DS17 wording wrinkle, closed in R4 Patch 2). All R3 new-issue residuals (J-R1, M-J-N2, M-J-N3, M-J-N4, N-B-3) closed in v4. cartan's two non-blocking v11.1-polish items closed in v4 Patch 7 ahead of v11.1 schedule.

**Ready for independent FORMALIS-arbiter Pareto declaration.** v4 is submission-ready: zero blocking residuals across all 6 R3 reviewers; the §15.6.1 declines-with-reasoning subsection installs the methodological gate jane_street J-R1 demanded; the §13.5.5 fault catalogue addendum installs the FoundationDB-discipline gate correctness N-B-3 demanded; the §11 DS17 / DS4 / DS19 invariant statements and the §6.3 CSDR penalty recipe and the §8.4.1 functoriality example are now Bourbaki-acceptable. The substantive R3 work — bijection Φ, DS19, LedgerReferenceInterpreter, TLA+ at v2/v3 fidelity, mutation testing 80%/100%, CSD outage protocol, broker-virtual semantics, threat model A1..A10 — was correct in v3 and is preserved unchanged in v4.

R4 was a one-pass surgical patch round: 7 patches across 8 sections; ~600 words additive content; no architectural change; voice and discipline preserved.
...
The 12 primary invariants (DS1, DS3, DS4, DS7, DS9, DS10, DS11a, DS11b, DS12, DS17, DS18, DS19 — DS19 added in v3) are the test the proposal must pass. Type-vs-runtime decomposition is named (§11 + §12.4). The §11.A restated v10.3 invariants are reasserted by reference, not duplicated. The §11.6 v10.3 regression-gate table provides P1..P20 verdicts; the six v10.3 characterisation tests (`char_1..char_6`) run unchanged in CI.
...
**v4 patches summary by section (additive over v3 — surgical R4 patch round).**

| v3 § | v4 patch | Length added |
|---|---|---|
| §3.Y.3 (line 663) | Patch 1 — rewritten without "wait, that is wrong" mid-derivation interjection | 0 net (rewrite) |
| §11 DS17 | Patch 2 — softened to per-row writer enumeration (jane_street M-J-N4) | ~10 lines |
| §13.5.5 NEW | Patch 3 — fault catalogue addendum: TA-DS-13 network partition + BugificationOperator (correctness N-B-3) | ~½ page |
| §6.3 CSDR penalty | Patch 4 — `csdr_penalty_obligation_id = hash_jcs(parent_obligation_id, failing_party_lei, accrual_date, schema_version)` (jane_street M-J-N3) | ~10 lines |
| §11 / §11.6 / §13.6.2 / §15.7 / §11 DS-table-header | Patch 5 — invariant count drift reconciled to 12 primary (jane_street M-J-N2) | ~6 lines net |
| §15.6 entry rewrite + §15.6.1 NEW | Patch 6 — R2/R3 findings declined with reasoning subsection (jane_street J-R1) | ~10 lines |
| §11 DS4 + §8.4.1 | Patch 7 — DS4 LHS extended to include `Failed/Instructed/PartiallySettled/BoughtIn/Compensated`; §8.4.1 example rewritten as morphism-collapse (cartan R2-M5 + R2-B4) | ~6 lines |
| §15.6.2 NEW | R3 closure record (panel verdicts + R4 closure mapping) | ~½ page |
| Total v4 delta | All R3 residual blockers + 3 jane_street minors + 2 cartan v11.1-polish items | ~600 words additive |

---

End of proposal_v4. Submitted to Phase 3 Round 4 final closure pass per the user's preferred multi-agent adversarial pattern. R4 is a surgical patch round (7 additive patches, no architecture change) closing the R3 residual blockers raised by jane_street (J-R1) and correctness (N-B-3) plus the three jane_street minors (M-J-N2, M-J-N3, M-J-N4) and cartan's two non-blocking v11.1-polish items (R2-M5, R2-B4). With R4 patches 1–7 applied, all R3 residual blockers are closed and the proposal is **ready for independent FORMALIS-arbiter Pareto declaration**.
