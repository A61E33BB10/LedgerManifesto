# Round 8 Scorecard — jane-street-cto
...
Neither file changed since my Round 7 sign-off (tex mtime 01:56, hs mtime 01:31 — identical
to Round 7). I re-read both fresh and re-verified the correctness chain by hand rather than
carry the grade forward on trust. The document still clears my bar and I re-stake my lens.

A competent quant engineer who has not read the review rounds can follow it in one careful
pass:
...
- §6 gives the three independent forcing constraints; §9 disposes of each rejected
  alternative by one forcing reason, not by the ordinal table (which is honestly flagged as
...
- **PT↔US invariant by construction.** `register` (sole introducer) writes both maps;
  ...
  the fresh id and rejects collision (`UnitAlreadyExists`). The `ghostSD` example validates
  vacuously then is rejected at apply with `UnknownUnit`; US is never fabricated for an
  unregistered unit.
...
- **`is_fungibility_preserving` argument naming** (§4 L506–509 types the second argument
  `TermsAmendment`; reference `FungibilityPredicate = ProductTerms -> TermsVersion ->
  Fungibility`). A reader cross-referencing tex to code meets two names for the amendment
  argument. Cosmetic — the amendment is a new terms version — not a correctness gap.
