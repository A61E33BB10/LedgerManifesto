...
Nothing in my domain has regressed; the unifications hold and every unified symbol is
introduced concretely before use. Correctness is preserved. The residuals are the same small
set, none cryptic, none cuttable without losing a deliberate function. I hold at 91 — neither
inflating nor cutting — and stake my lens on A.
...
- **One conservation structure, not a branch per instrument.** C2 (254–266) states
  ...
  are *instances*. The reference realises it as `conserved :: FieldWrite h -> PosDelta`
  (ref. 271–276) `foldMap`-ed to `mempty` in `validate` (ref. 308–314); C9 is the empty fold
  — `mempty` over no wallets — with no special case. Prose and code coincide. The Dirac
  moment of the document.
- **The conserved fields are a *product*, demonstrated not asserted.** `PosDelta` carries
  ...
  landing at the identity componentwise. `balance` exists precisely to give the product a
  second component and C11 a second conserved field with a writer (`transfer`) distinct from
  `ac`'s — which `hwm`/`entry_nav` (non-conserved) cannot show. Prose is transparent about
  this pedagogical role (122–127, 203–205) and excludes `balance` from the economic
  inventory by intent.
...
- **`FieldWrite h` GADT *is* the C11 writer table** (ref. 200–205:
  `WAc`/`WAcTrade`/`WBalance`/`WHwm`/`WEntryNav`, then `SomeWrite` erasing the index, 212–213).
  Type error at authorship, index erased at the stored row (signal S3, ref. 489–499). The
  prose's `erase = fmap (map SomeWrite)` and `settleHandler :: ... -> Map WalletId
  [FieldWrite 'Settle]` (809–812) match ref. 231–239 verbatim. The commented `_c11_bad`
  (ref. 462–463) witnesses the rejected cross-handler write.
- **P3 typechecks as a homomorphism.** `replay (xs <> ys) = replay xs >=> replay ys`
  ...
  (ref. 388–397). Checkpoint independence is a consequence of C1(b), not a test.
...
- **C1–C12 are in index order, not first-appearance order** (212–218): a reader meets C2,
  then C1, C11, C12, C3, … forcing the disclaimer. For the target reader the "stable tag"
  justification reads as out-of-order labels with a caveat. The §6 index resolves every
  lookup and the rule is stated once and obeyed. Friction, not crypticness.
- **Prose/reference accessor-name drift.** Prose writes `product_terms(u)` /
  ...
  `position`. Map names are bridged (161–164 → `ledgerUS`/`ledgerPS`); accessor names are
  not. snake_case = spec convention, camelCase = Haskell; a competent reader maps them on
  sight. Cosmetic.
- **`balance` is a deliberately demonstrative conserved field** (122–127, 203–205). A
  ...
  a writer distinct from `ac`'s. The document is explicit about its non-economic role. A
...
- **Signals S1–S4 are defined in the reference comment block** (ref. 465–507) and referenced
  parenthetically earlier. Announced at 216 and substantively stated inline at each call
