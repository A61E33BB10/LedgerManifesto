...
round 8 and introduces no new one-pass hazard.
...
- **Label/ref integrity.** No duplicate `\label`. Every `\ref` resolves (zero MISSING).
  Spine has no dangling pointer.
- **Accessor name is single.** `position(w,u)` is the only accessor name, at exactly three
  sites (C1(a), §4.4, §7). `position_state(` appears nowhere. One name, one operation.
...
- **Prose names match the reference exactly.** `ledgerUS`, `ledgerPS`, `zeroP`,
  ...
  `erase`, `_c11_bad` all present in `reference/StatesHome.hs` as named in the prose. A reader
  crossing from §3/§4 prose into the §13 listing meets no renamed entity.
...
- **C11 vs C2 two-axis disambiguation** (C11 body). Field-writers vs event classes named
