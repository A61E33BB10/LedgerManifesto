# Round 10 Scorecard — henri-cartan
...
I awarded A in Rounds 3–9 and staked my lens each time. For Round 10 I re-read the full text
against the A bar (a competent quant engineer with no prior exposure to the 27 review rounds,
one careful pass) rather than carrying the prior verdict, and re-confirmed the one external
dependency my lens rests on: the included listing `reference/StatesHome.hs` (598 lines,
present) still carries the `[S1]`–`[S4]` notes (468, 482, 489, 501) and the `_c11_ok_*` /
commented `_c11_bad` witnesses (457–463) that the prose cites. The bar holds.
...
- **Definitions precede use.** §2 (Notation, 90–146) fixes every document-local symbol before
  ...
  `Delta f(w,u)` (120–121); the `conserved field` enumeration (122–127); `flat`/`0_P` grounded
  in `zeroP` (128–131); `StateDelta` (132–133). C2 (254) and C3 (424) consume these with no
  forward dependency on an undefined term.
...
- **Signal vocabulary glossed before use and resolvable on the page.** Line 216 says what
  ...
  at C11 (312). The `lstinputlisting` (827) makes the citations resolve on the page;
...
- **Correctness preserved.** Every claim is proved in-line, made structural by the encoding
  (§12 maps each C-condition to a constructor, 800–825), or precisely deferred to a labelled
  external citation (v10.3 §11). §10 names exactly seven invariants (P1, P3, P5, P6, P7, P9,
  P10), consistent with the abstract's "seven of the ten." The `unrepresentable` claim is
  carefully qualified (668–676): conservation is a value-level check (signal S4), not a type
  fact. The `balance` field is flagged as the demonstrative second conserved field — not an
  economic datum — at every occurrence (122–127, 203–205, 308).
