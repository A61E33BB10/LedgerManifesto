...
% Discharges Constitution v1.1 §14.1 and §14.2. Restates as minima the derivations of
...
This chapter discharges \S 14.1 and \S 14.2 of the constitution.
...
\item \textbf{Every datum carries provenance.} Every datum on the record must carry its
  provenance --- source, observation time, and basis frame --- and no valuation may read a
  datum that does not (Chapter~\ref{ch:marketdata}, \S\ref{sec:obs-door}). A bare read
  against a live feed leaves no record and is unreproducible; it must be refused.
\item \textbf{Kinds are registered before they cross.} Every datum's kind must be
  registered, with the dimension of each component --- price, proportional, or quantity ---
  before any datum of that kind crosses; a datum of an unregistered kind must be refused at
...
\item \textbf{An operator is declared for every pairing.} For every (registered datum kind,
...
\item \textbf{Originals preserved, adjusted computed at read.} Every observed datum must be
...
\item \textbf{Each failure regime has its determined disposition.} Each boundary failure
  --- missing datum (W1), late datum (W2), contradictory sources (W3), unmappable event
  ...
  (Chapter~\ref{ch:marketdata}, \S\ref{sec:failures}). In particular a single source for a
  datum that materially affects valuation is insufficient: such a datum must be drawn from
...
\item \textbf{Declared agreement terms are reconciled at the boundary.} Every declared
  ...
  transcription error repaired by terms amendment plus a compensating rebooking, never a
...
The boundary rests on two named trust assumptions and no other: TA-KIND, that a kind
...
(Chapter~\ref{ch:marketdata}); and TA-TERMS, that a declared agreement term faithfully
transcribes the executed agreement (B7), owned by the same governance and detected by
boundary reconciliation against the counterparty's own record. Everything else that
crosses is recorded, framed, and replayable.
...
confirm that no
datum lacks provenance, that no unregistered kind and no undeclared pairing crosses, that
