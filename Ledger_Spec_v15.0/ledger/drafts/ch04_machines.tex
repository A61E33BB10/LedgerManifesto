...
This chapter discharges \S5 of the constitution.
...
corporate-action notices, fixings and other observations --- and records them
at the boundary. Second, for each event, arrived or emitted, it fires the
...
contracts; it never writes the ledger; its output is only proposals. A proposal
is not a fact --- it becomes one only if admitted.
...
Idempotence is the door's own defence, owed to no one upstream: every proposed
transaction carries an identifier derived from its cause --- the recorded event
that produced it --- so a retried, duplicated, or late proposal for an
already-processed event is recognised and committed exactly once.
...
The one-touch \mbox{OT-1} pays \USD{1{,}000{,}000} when OMEGA's official close is at or
below the barrier 80.00 --- a discretely-monitored (closing-price) one-touch;
it is written by \mbox{W-BANK} and
held by \mbox{W-ALPHA}, and its terms declared the watch at registration: the
recorded official closes of OMEGA against the barrier.
...
the touch event and the current projection. The contract reads the terms
(payout \USD{1{,}000{,}000}), the holder (\mbox{W-ALPHA}), and the unit's state
(\emph{live}), and returns one transaction.
...
Had a duplicate proposal been produced anyway, its identifier names an
already-processed cause and the door commits it zero further times. Two
