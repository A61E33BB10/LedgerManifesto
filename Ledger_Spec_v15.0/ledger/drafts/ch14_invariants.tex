...
This chapter discharges \S 11 and \S 12 of the constitution.
...
\subsection{Conservation per (unit, coordinate)}

The central law is a conservation law, and it is stated per coordinate, not per
unit. For a wallet $w$, a unit $u$, and a coordinate $c\in\{\text{owned, lent,
posted}\}$, write $\mathrm{bal}_c(w,u)$ for the balance obtained by folding the
log (Chapter~\ref{ch:picture}).

\begin{theorem}[Conservation]\label{thm:conservation}
For every unit $u$, every coordinate $c$, and every position in the log,
\[
\sum_{w}\ \mathrm{bal}_c(w,u)\ =\ 0,
\]
...
Issuance is the first paired-leg owned move out of the issuing wallet;
retirement is the log position after which every wallet's vector for $u$ is the
zero vector. The per-unit summed law --- $\sum_w\sum_c \mathrm{bal}_c(w,u)=0$ ---
is a corollary, never the primitive: the per-coordinate form is primitive
because it alone forbids a cross-coordinate rebooking that fabricates custody on
the posted plane while conserving the gross sum.

\begin{proof}
Induction on the log. \emph{Base:} the empty log gives every $(u,c)$ plane a sum
of zero. \emph{Step:} the only operation that writes a balance is an atomic move,
which debits one wallet and credits another in the same coordinate of the same
unit by the same positive quantity $q$, adding $(-q)+(+q)=0$ to the sum; the
other three kinds of change a transaction bundles --- unit-state, position-state,
terms --- touch no balance. The sum is therefore invariant across every admitted
transaction, and remains $0$.
\end{proof}

\begin{lstlisting}
conserved :: Log -> Unit -> Coord -> Bool
conserved l u c = sum [ balance c w u (fold l) | w <- wallets l ] == 0
...
that share no (wallet, unit, coordinate) --- and every coordinate sum is
...
conservation per (unit, coordinate); no reconciliation enforces the zero-sum ---
...
\begin{secondtelling}
Fix $(u,c)$. The map sending a log to the wallet-indexed integer vector of
...
idempotent   :: Tx  -> Ledger -> Bool   -- re-applying a seen cause is a no-op
\end{lstlisting}
...
\item \textbf{Idempotence.} Every proposed transaction carries an identifier
  derived from its cause (Chapters~\ref{ch:machines} and~\ref{ch:picture}); a
  retried or late-arriving proposal for an already-processed cause is recognised
  and committed once. Applying a transaction whose identifier is already on the
  log changes nothing.
\end{itemize}
...
instrument. Every recorded datum carries the basis frame it was observed in
...
price excludes it; the two are one datum kind read in states separated by the
...
Section \S 12's guarantees are theorems of the fold.

\begin{theorem}[Time travel]\label{thm:timetravel}
Every historical state of the ledger is the fold of a log prefix. For any past
position $t$, the ledger as of $t$ is $\mathrm{fold\ step}$ over the prefix up to
$t$, and every view --- a balance, a NAV, a report --- is a further fold of that
same prefix. Because the fold reads only committed transactions and never
re-executes contracts, the historical state is independent of the machinery that
produced it and reconstructible at any time, forever.
\end{theorem}
...
the recorded fixings and comparing --- and this catalogue does not restate the
test.
...
\]
the received ray entering with positive sign, and the analogue with lent in place
of posted.
\end{invariant}

The sum over signed balances lets received mass offset re-posted mass: a wallet
that received $100$ under one agreement and re-posted $60$ under another nets to
coverage $40$, within the bound, so a rehypothecation chain passes and is bounded
by possession. The $\max$ binds only the posting direction, so a negative-owned
...
\midrule
Possession coverage & $\sum_G \mathrm{posted}_G \le \max(\mathrm{owned},0)$ & never (atomic, instantaneous) & invariant, checked at the door \\
...
\emph{Setup.} W-ALPHA owns OT-1 (payout \USD{1{,}000{,}000}, marked
\USD{400{,}000}), pledged under security-interest agreement G to counterparty
wallet B at a $50\%$ valuation percentage: W-ALPHA posted $+1$, B posted $-1$;
collateral value to B \USD{200{,}000} against exposure \USD{150{,}000}. OMEGA
prints an official close of $79.00 \le$ the barrier $80.00$.
...
wrong frame, collateral delivered that was never possessed --- are not caught
here; they are unreachable. What cannot be represented cannot break.
