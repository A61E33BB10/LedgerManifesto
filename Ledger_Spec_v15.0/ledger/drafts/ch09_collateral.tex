...
the re-booking rule creates the gap it must close. Re-booking at instruction
...
of this chapter's close --- never a silent breach. That fails obligation is, like
every obligation, a valued unit, minted --- once the reversal is refused --- by the
settlement-fail event's contract, symmetric to the market-claim unit the record-date
firing mints (Chapter~\ref{ch:settlement}; the projection writes nothing). With its
...
any other declared datum, not a new mechanism.
