...
immutable log's total order for a unit's fixings is the recorded observation index~$k$ ---
the fixing sequence --- not the order in which prints arrive; replay folds the accrual in
that order and is deterministic. A late or out-of-order print is inserted at its
observation position~$k$, and the accumulator tail $A_{k},A_{k+1},\dots$ is recomputed from
$k$ forward --- identical to timely arrival because the fold is over the ordered sequence,
