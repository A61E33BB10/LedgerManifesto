...
(Chapter~\ref{ch:collateral}); the component dimensions of the datum-kind registry and the
...
immutable log's total order for a unit's fixings is the recorded observation index~$k$, so
the proposal is inserted at its observation position~$k$ and the order-dependent accumulator
tail $A_{k},A_{k+1},\dots$ is re-sequenced and recomputed from $k$ forward. \emph{Replay
...
observation are deterministic --- each datum crosses the boundary once, as a recorded
...
mutator; a duplicate firing is inert and a late firing folds in at its observation index ---
identical when reordered against unrelated events, its tail recomputed when reordered within
its unit's own sequence; firing~127 recomputes bit-for-bit; and the
