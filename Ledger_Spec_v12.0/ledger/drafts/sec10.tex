...
\begin{remark}[Balance-as-fold lens, optional]
Plain reading: balances are not stored and maintained; they are recomputed by folding the move stream. Concatenating two runs of events and folding gives the same result as folding each and adding---so balances of any sub-period compose into the whole.

\emph{Optional.} \lstinline!balances = foldMap contribution : [Move] -> Balances! is a monoid homomorphism (\lstinline!balances (xs <> ys) = balances xs <> balances ys!) into the \lstinline!Balances! monoid; this single law gives deterministic replay, $O(k)$ incremental snapshots, and conservation together.
\end{remark}
...
Unit state read during substantiation comes from UnitStatus, a materialised projection of the same log. UnitStatus holds one value per unit, shared---read identically by every holder. Its value changes over time, but UnitStatus is not a separate source of truth: the immutable event log is. Every change is caused by a logged event; the stored value is overwritten in place only as a read cache. Replaying the events up to any point rebuilds the exact value that held then. Nothing is lost by overwriting, and there is no other way to change the value. A value exists from registration onward.
